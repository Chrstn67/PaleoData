...
const FictionDetail = () => {
    ...
    return (
      <div className="article-detail-page">
        ...
        <div className="article-not-found">
          <h1>Article non trouvé</h1>
          <Link to="/documentation/fiction" className="navigation-link">
            ← Retour aux fictions
          </Link>
        </div>
      </div>
  ...
  const sortedData = [...fictionsData].sort((a, b) => a.notion.localeCompare(b.notion));
            ...
            <div className="article-detail-illustrations">
              {fiction.illustrations.map((illustration, index) => (
                <div key={index} className="illustration-item">
                  <img src={illustration || '/placeholder.svg'} alt={fiction.alt?.[index] || fiction.notion} />
                  {fiction.alt?.[index] && <p className="illustration-caption">{fiction.alt[index]}</p>}
                </div>
              ))}
            </div>
          )}
          ...
          <div className="article-detail-content">
            <div className="article-detail-speech-bubble">
              <div className="article-detail-speech-bubble-content">
                <div dangerouslySetInnerHTML={{ __html: fiction.explications }} />
              </div>
            </div>
          </div>
        </div>
