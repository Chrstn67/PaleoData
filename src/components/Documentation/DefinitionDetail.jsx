...
const DefinitionDetail = () => {
    ...
    return (
      <div className="article-detail-page">
        ...
        <div className="article-not-found">
          <h1>Article non trouvé</h1>
          <Link to="/documentation/definitions" className="navigation-link">
            ← Retour aux définitions
          </Link>
        </div>
      </div>
  ...
  const sortedData = [...definitionsData].sort((a, b) => a.notion.localeCompare(b.notion));
            ...
            <div className="article-detail-illustrations">
              {definition.illustrations.map((illustration, index) => (
                <div key={index} className="illustration-item">
                  <img src={illustration || '/placeholder.svg'} alt={definition.alt?.[index] || definition.notion} />
                  {definition.alt?.[index] && <p className="illustration-caption">{definition.alt[index]}</p>}
                </div>
              ))}
            </div>
          )}
          ...
          <div className="article-detail-content">
            <div className="article-detail-speech-bubble">
              <div className="article-detail-speech-bubble-content">
                <div dangerouslySetInnerHTML={{ __html: definition.explications }} />
              </div>
            </div>
          </div>
        </div>
