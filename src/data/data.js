const data = [
  {
    nom: 'Acadagnostus',
    etymologie: 'Ailes géantes (en référence aux deux épines dorsales)',
    image_url: './assets/images/Acadagnostus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Trilobita',
      sous_classe: '',
      infra_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Agnostida',
      sub_ordre: 'Agnostina',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Agnostoidea',
      famille: 'Peronopsidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Acadagnostus',
      espèces:
        'Acadagnostus acadicus, Acadagnostus ancisa, Acadagnostus australis, Acadagnostus certus, Acadagnostus karatauensis, Acadagnostus rakuroensis, Acadagnostus syrma',
    },
    regime_alimentaire: 'Carnivore filtreur',
    habitat: 'Fond des océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Miaolingien',
      stage: 'Wuliuen',
      apparition: -509,
      extinction: -505,
    },
    description:
      "Acadagnostus est un animal avec une tête ronde et lisse, sans épines. Le devant de sa tête est étroit, tandis que le dos de sa tête présente des rainures qui ressemblent à des fosses ou à un sillon peu profond, droit ou légèrement courbé vers l'arrière. Il a une bosse au milieu de sa tête ou légèrement en arrière. Sa queue est presque carrée, avec un bord plat qui peut parfois s'élargir vers l'arrière, et elle a une légère encoche. Deux petites épines se trouvent à l'arrière dans les coins extérieurs. Son dos présente un large sillon frontal. Sa colonne vertébrale a des côtés droits ou coniques, mais le deuxième segment n'est pas plus étroit que les autres. Les rainures qui traversent la colonne vertébrale sont soit absentes, soit peu marquées. Les segments sont marqués par des différences dans la hauteur de la colonne vertébrale. Il y a une grosse bosse au milieu, et il y a un sillon court à l'arrière.",
    autres_infos: {
      taille: {
        longueur: '2 centimètres',
        hauteur: '1 centimètre',
      },
      poids: '10 grammes',
    },
    decouverte: {
      date: '1840',
      lieu: 'États-Unis',
    },
  },
  {
    nom: 'Acanthostega',
    etymologie: 'Abri épineux',
    image_url: './assets/images/Acanthostega.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Acanthostegidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Acanthostega',
      espèces: 'Acanthostega gunnari',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Rivières, terre ferme',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Supérieur',
      stage: 'Famennien',
      apparition: -372,
      extinction: -372,
    },
    description:
      "Acanthostega possédait à la fois des caractéristiques de poissons et d'amphibiens. Il avait des membres osseux ressemblant à des nageoires, mais aussi des doigts et des orteils avec des os individuels. Cependant, il était encore bien adapté à la vie aquatique et ne pouvait pas se déplacer efficacement sur la terre ferme.",
    autres_infos: {
      taille: {
        longueur: '50-60 centimètres',
        hauteur: 'env. 20 centimètres',
      },
      poids: '2-3 kilogrammes',
    },
    decouverte: {
      date: '1933',
      lieu: 'Groenland',
    },
  },
  {
    nom: 'Acherontiscus',
    etymologie: 'Référence à l’Achéron, une rivière de la mythologie grecque',
    image_url: './assets/images/Acherontiscus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Adelospondyli',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sub_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: '',
      espèces: 'Acherontiscus caledoniae',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Zones humides',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Carbonifère',
      epoque: 'Mississippien',
      stage: 'Viséen',
      apparition: -347,
      extinction: -315,
    },
    description:
      'Acherontiscus avait un corps allongé avec des nageoires et des écailles ressemblant à celles des poissons modernes',
    autres_infos: {
      taille: {
        longueur: '15-20 cm',
      },
      poids: '',
    },
    decouverte: {
      date: '1987',
      lieu: 'Écosse',
    },
  },
  {
    nom: 'Acleistorhinus',
    etymologie: 'Nez non-fermé',
    image_url: './assets/images/Acleistorhinus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Procolophonomorpha',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Acleistorhinidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Acleistorhinus',
      espèces: 'Acleistorhinus pteroticus',
    },
    regime_alimentaire: 'Insectivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Permien',
      epoque: 'Guadalupien',
      stage: 'Roadien',
      apparition: -273.6,
      extinction: -271.6,
    },
    description:
      "Acleistorhinus était caractérisé par son crâne court et large, son corps massif et ses membres robustes. Il possédait une carapace osseuse qui lui offrait une protection contre les prédateurs. Il se nourrissait probablement d'insectes et d'autres petits animaux. Acleistorhinus fait partie des nombreux reptiles qui ont vécu avant l'apparition des dinosaures.",
    autres_infos: {
      taille: {
        longueur: '1,5 mètre',
        hauteur: '0,5 mètre',
      },
      poids: '100 kilogrammes',
    },
    decouverte: {
      date: '1969',
      lieu: 'Afrique du Sud',
    },
  },
  {
    nom: 'Akmonistion',
    etymologie: "Voile en forme d'enclume",
    image_url: './assets/images/Akmonistion.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Chondrichthyes',
      sous_classe: '',
      division: '',
      super_ordre: 'Symmoriiformes',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Stethacanthidae',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Akmonistion',
      espèces: 'Akmonistion zangerli',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Carbonifère',
      epoque: 'Mississippien',
      stage: 'Serpukhovien',
      apparition: -330,
      extinction: -323,
    },
    description:
      'Akmonistion est un animal aquatique ressemblant à un requin mais faisait partie de la famille des poissons cartilagineux. Son aileron dorsal ressemble à une enclume.',
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '60 centimètres',
      },
      poids: '100 kilogrammes',
    },
    decouverte: {
      date: '2001',
      lieu: 'Écosse',
    },
  },
  {
    nom: 'Amiskwia',
    etymologie: '--',
    image_url: './assets/images/Amiskwia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Amiskwia',
      espèces: 'Amiskwia sagittiformis, Amiskwia sinica',
    },
    regime_alimentaire: 'Inconnu',
    habitat: 'Fond des océans ou mers peu profondes',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 3',
      apparition: -521,
      extinction: -509,
    },
    description:
      "Amiskwia est un animal invertébré considéré comme un membre énigmatique du règne animal, car sa classification précise reste incertaine. <br /> Amiskwia avait une forme allongée et ressemblait à un ver ou à un lombric, avec un corps segmenté et des appendices. Il possédait également des structures en forme de couronne à l'extrémité antérieure, suggérant peut-être une fonction alimentaire spécifique. Bien que son mode de vie exact soit inconnu, Amiskwia était probablement vivant au fond des océans ou des mers peu profondes.",
    autres_infos: {
      taille: {
        longueur: '10 à 20 cm',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1959',
      lieu: 'Lac Amisk, Saskatchewan, Canada',
    },
  },
  {
    nom: 'Anglaspis',
    etymologie: 'Écaille angulaire',
    image_url: './assets/images/Anglaspis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Pteraspidomorphi',
      sous_classe: 'Heterostraci',
      division: '',
      super_ordre: '',
      ordre: 'Cyathaspidiformes',
      sous_ordre: 'Cyathaspidida',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Ariaspidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Anglaspis',
      espèces: '',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Silurien',
      epoque: 'Pridoli',
      stage: 'Pridolien',
      apparition: -425,
      extinction: -415,
    },
    description:
      "Anglaspis est un genre de poissons fossiles appartenant à la classe des placodermes. Il se distinguait par son corps recouvert d'une armure osseuse et sa tête munie de plaques protectrices. Anglaspis était un prédateur marin , doté de mâchoires puissantes pour saisir et broyer leur proie. Il était largement répandu dans les océans de l'époque.",
    autres_infos: {
      taille: {
        longueur: "jusqu'à 1 mètre",
        hauteur: "jusqu'à 50 centimètres",
      },
      poids: 'environ 10 kilogrammes',
    },
    decouverte: {
      date: '1856',
      lieu: 'Angleterre',
    },
  },
  {
    nom: 'Anomalocaris',
    etymologie: 'Crevette anormale',
    image_url: './assets/images/Anomalocaris.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: 'Metazoa',
      rameau: 'Ecdysozoa',
      infra_règne: 'Panarthropoda',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: 'Chelicerata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Dinocaridida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Radiodonta',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Anomalocarididae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Anomalocaris',
      espèces:
        'Anomalocaris canadensis, Anomalocaris whiteavesi, Anomalocaris gigantea, Anomalocaris cranbrookensis, Anomalocaris daleyae',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 3',
      apparition: -520,
      extinction: -500,
    },
    description:
      "Anomalocaris possédait un corps allongé avec une paire d'yeux composés, une bouche munie de nombreuses pointes et des appendices en forme de nageoires. <br /> Cet animal est considéré comme l'un des premiers prédateurs de grande taille de l'histoire de la vie sur Terre.",
    autres_infos: {
      taille: {
        longueur: '2-4 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1984',
      lieu: 'Canada',
    },
  },
  {
    nom: 'Arandaspis',
    etymologie: "Vient du lieu de sa découvete, près duquel habitent les Aranda, une tribu arborigène d'Australie",
    image_url: './assets/images/Arandaspis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: 'Agnatha',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Pteraspidomorphi',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Arandaspida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Arandaspididae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Arandaspis',
      espèces: 'Arandaspis prionotolepis',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Ordovicien',
      epoque: 'Inférieur',
      stage: 'Trémadocien',
      apparition: -480,
      extinction: -470,
    },
    description:
      "Arandaspis est un genre éteint de poissons agnathes (sans mâchoires). <br /> Arandaspis possédait un corps allongé et dépourvu de mâchoires, avec une peau couverte de plaques osseuses, ce qui en fait l'un des premiers poissons à posséder une armure. Il se déplaçait dans les océans anciens en filtrant les particules alimentaires de l'eau.",
    autres_infos: {
      taille: {
        longueur: '15 centimètres',
        hauteur: '',
      },
      poids: '150 grammes',
    },
    decouverte: {
      date: '1959',
      lieu: 'Australie centrale',
    },
  },
  {
    nom: 'Arthropleura',
    etymologie: 'Côté articulé',
    image_url: './assets/images/Arthropleura.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: 'Myriapoda',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Diplopoda',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Arthropleurida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Arthropleuridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Arthropleura',
      espèces: [
        'Arthropleura armata',
        'Arthropleura longicollum',
        'Arthropleura minor',
        'Arthropleura prisca',
        'Arthropleura prorsicauda',
        'Arthropleura rhenana',
        'Arthropleura sibericorum',
        'Arthropleura spinosa',
        'Arthropleura sternbergii',
      ],
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Carbonifère',
      epoque: 'Mississippien',
      stage: 'Viséen',
      apparition: 345,
      extinction: 290,
    },
    description:
      "Arthropleura était un animal ressemblant à un mille-pattes. Arthropleura possédait un corps segmenté avec de nombreuses pattes, une carapace externe protectrice et des mandibules pour se nourrir. C'était l'un des plus grands arthropodes terrestres connus de l'histoire de la Terre. <br /> Bien qu'impressionnant, il devait se nourrir uniquement de végétaux.",
    autres_infos: {
      taille: {
        longueur: '2.50-3 mètres',
        hauteur: '20 centimètres',
      },
      poids: '50-100 kilogrammes',
    },
    decouverte: {
      date: '1854',
      lieu: 'Angleterre',
    },
  },
  {
    nom: 'Aysheaia',
    etymologie: '--',
    image_url: './assets/images/Aysheaia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: 'Eumetazoa',
      rameau: 'Bilateria',
      infra_règne: 'Ecdysozoa',
      super_embranchement: 'Panarthropoda',
      embranchement: 'Lobopodia',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Xenusia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Protonychophora',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Aysheaiidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Aysheaia',
      espèces: 'Aysheaia pedunculata',
    },
    regime_alimentaire: 'Carnivore filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Miaolingien',
      stage: 'Wuliuen',
      apparition: -509,
      extinction: -505,
    },
    description:
      "Aysheaia était caractérisé par son corps segmenté et allongé, avec des appendices en forme de pattes sur chaque segment. Ces créatures étaient dépourvues de membres articulés et possédaient une bouche située à l'extrémité antérieure de leur corps. ",
    autres_infos: {
      taille: {
        longueur: '1-6 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1911',
      lieu: 'Schistes de Burgess, Canada',
    },
  },
  {
    nom: 'Birkenia',
    etymologie:
      "Dérivé de 'Birken' en référence à la région de Birkenes en Norvège, où les premiers fossiles de cet animal préhistorique ont été découverts",
    image_url: './assets/images/Birkenia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Anaspida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Birkeniiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Birkeniidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Birkenia',
      espèces: 'Birkenia campbelli, Birkenia elegans, Birkenia robusta',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Silurien',
      epoque: 'Llandovery',
      stage: 'Télychien',
      apparition: -438,
      extinction: -411,
    },
    description:
      "Birkenia se caractérise par son corps allongé, en forme de poisson, avec une structure externe constituée d'une série de pièces dentaires minuscules appelées 'conodontes'. Ces conodontes étaient disposés dans une bouche qui leur permettait de se nourrir de proies plus petites dans leur environnement marin.",
    autres_infos: {
      taille: {
        longueur: '30-60 centimètres',
        hauteur: '10-20 centimètres',
      },
      poids: '1-2 kilogrammes',
    },
    decouverte: {
      date: '1912',
      lieu: 'Birkenes, Norvège',
    },
  },
  {
    nom: 'Boreaspis',
    etymologie: 'Ecailles du Nord',
    image_url: './assets/images/Boreaspis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: 'Vertebrata',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Osteostraci',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Benneviaspidida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Boreaspididae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Boreaspis',
      espèces:
        'Boreaspis batoides, Boreaspis macrorhynchus, Boreaspis puella, Boreaspis rostrata, Boreaspis spinicornis, Boreaspis triangularis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Eaux douces',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Inférieur',
      stage: 'Praguien',
      apparition: -410,
      extinction: 394,
    },
    description:
      "Boreaspis possédait un corps massif et allongé, recouvert d'écailles osseuses ou de plaques, qui offraient une protection contre les prédateurs. Sa tête était dotée d'une sorte de bouclier épais, qui était également osseux et qui lui permettait de mieux se défendre contre les attaques. Ces caractéristiques faisaient de lui un prédateur puissant et bien adapté à son environnement aquatique.",
    autres_infos: {
      taille: {
        longueur: '1-2 mètres',
        hauteur: '30-40 centimètres',
      },
      poids: '10-20 kilogrammes',
    },
    decouverte: {
      date: '1859',
      lieu: 'Spitzberg, Norvège',
    },
  },
  {
    nom: 'Bothriolepis',
    etymologie: 'Ecailles en forme de fosse',
    image_url: './assets/images/Bothriolepis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Placodermi',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Placodermi',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Bothriolepididae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Bothriolepis',
      espèces:
        'Bothriolepis ornata Eichwald, Bothriolepis africana Long et al., Bothriolepis alexi Young, Bothriolepis alvesiensis Stensio, Bothriolepis amankonyrica Malinovskaja, Bothriolepis antarctica Woodward, Bothriolepis askinae Young, Bothriolepis babichevi Malinovskaja, Bothriolepis barretti Young, Bothriolepis bindareii Long, Bothriolepis canadensis Whiteaves, Bothriolepis cellulosa Pander, Bothriolepis ciecere Lyarskaja, Bothriolepis coloradensis Eastman, Bothriolepis cristata Traquair, Bothriolepis cullodenensis Long, Bothriolepis dairbhrensis Dupret et al., Bothriolepis darbiensis Denison, Bothriolepis dorakarasugensis Moloshnikov, Bothriolepis evaldi Lyarskaja, Bothriolepis favosa Agassiz, Bothriolepis fergusoni Long, Bothriolepis gigantea Traquair, Bothriolepis gippslandiensis Hills, Bothriolepis grenfellensis Johanson, Bothriolepis groenlandica Heintz, Bothriolepis hayi Miles, Bothriolepis heckeri Luksevics, Bothriolepis hicklingi Miles, Bothriolepis hydrophila Agassiz, Bothriolepis jani Luksevics, Bothriolepis jarviki Stensio, Bothriolepis jazwicensis Szrek, Bothriolepis jeremijevi, Bothriolepis karawaka Young, Bothriolepis kassini Malinovskaja, Bothriolepis kohni Young, Bothriolepis kwangtungensis Pan, Bothriolepis laverocklochensis Miles, Bothriolepis leptocheira Traquair, Bothriolepis lochangensis Pan, Bothriolepis lohesti Leriche, Bothriolepis macphersoni Young, Bothriolepis macrocephala Egerton, Bothriolepis maeandrina, Bothriolepis markovskii Moloshnikov, Bothriolepis mawsoni Young, Bothriolepis maxima Gross, Bothriolepis meandrina Horrmann, Bothriolepis minor Newberry, Bothriolepis nielseni Stensio, Bothriolepis nikitinae Malinovskaja, Bothriolepis nitida Leidy, Bothriolepis niushoushanensis Pan & Wang,Bothriolepis obesa Traquair, Bothriolepis obrutschewi Gross, Bothriolepis ornata Andrews, Bothriolepis panderi Lahusen, Bothriolepis paradoxa Agassiz, Bothriolepis pavariensis Lyarskaja, Bothriolepis perija Young & Moody, Bothriolepis portalensis Young, Bothriolepis prima Gross, Bothriolepis rex Downs et al., Bothriolepis sanzarensis Moloshnikov, Bothriolepis shaokuanensis Liu, Bothriolepis sibrica Obruchev, Bothriolepis sinensis Chi, Bothriolepis sosnensis Moloshnikov, Bothriolepis stevensoni Miles, Bothriolepis tastenica Malinovskaja, Bothriolepis tatongensis Long & Werdelin, Bothriolepis taylori Miles, Bothriolepis traquairi Bryant, Bothriolepis trautscholdi Jaekel, Bothriolepis tungseni Chang, Bothriolepis turanica Obruchev, Bothriolepis verrucosa Young & Gorter, Bothriolepis virginiensis Weems et al., Bothriolepis volongensis Lyarskaja & Luksevics, Bothriolepis vuwae Young, Bothriolepis warreni Long, Bothriolepis wilsoni Miles, Bothriolepis yeungae Johanson, Bothriolepis yunnanensis Liu, Bothriolepis zadonica Obrucheva',
    },
    regime_alimentaire: 'Détritivore',
    habitat: 'Fond des océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Moyen',
      stage: 'Givétien',
      apparition: -387,
      extinction: -359,
    },
    description:
      "Bothriolepis possédait une carapace épaisse et plate, dotée de rangées d'écailles articulées qui lui offraient une protection solide. Son corps était recouvert de plaques osseuses, et il était muni de nageoires pectorales et pelviennes qui lui permettaient de se déplacer dans l'eau. Cet animal avait une tête caractéristique avec de grands yeux sur le dessus, une bouche infère, et de petits appendices en forme de cornes devant ses yeux, qui étaient probablement utilisés pour fouiller le fond de l'océan à la recherche de nourriture. On le retrouve pratiquement partout autour du globe.",
    autres_infos: {
      taille: {
        longueur: '30-100 centimètres',
        hauteur: '10-30 centimètres',
      },
      poids: '1-10 kilogrammes',
    },
    decouverte: {
      date: '1840',
      lieu: 'Canada',
    },
  },
  {
    nom: 'Canadaspis',
    etymologie: 'Bouclier canadien',
    image_url: './assets/images/Canadaspis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Hymenocarina',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Canadaspididae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Canadaspis',
      espèces: 'Canadaspis perfecta, Canadaspis laevigata',
    },
    regime_alimentaire: 'Carnivore filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 3',
      apparition: -521,
      extinction: -505,
    },
    description:
      "Canadaspis possédait un corps allongé et aplati, recouvert d'un bouclier externe en forme de bouclier. Ce bouclier était composé de segments articulés, ce qui lui permettait une certaine souplesse. L'animal avait également des antennes pour détecter son environnement et de nombreuses pattes pour se déplacer dans l'eau. On pense que Canadaspis se nourrissait de particules en suspension dans l'eau et qu'il était une proie pour d'autres animaux marins de cette époque.",
    autres_infos: {
      taille: {
        longueur: '10 à 15 cm',
        hauteur: '5 à 7 cm',
      },
      poids: 'env. 10 grammes',
    },
    decouverte: {
      date: '1912',
      lieu: 'Canada',
    },
  },
  {
    nom: 'Cephalaspis',
    etymologie: "Tête d'écailles",
    image_url: './assets/images/Cephalaspis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Osteostraci',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Cephalaspis',
      espèces: 'Cephalaspis lyelli',
    },
    regime_alimentaire: 'Détritivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Inférieur',
      stage: 'Lochkovien',
      apparition: -415,
      extinction: -398,
    },
    description:
      "Cephalaspis était caractérisé par sa tête en forme de bouclier, couverte d'une grande plaque osseuse. Son corps était allongé et recouvert d'écailles. En raison de son corps cartilagineux et de sa faible capacité à se déplacer rapidement, Cephalaspis était probablement une proie facile pour les prédateurs marins de l'époque. Pour se protéger, il pouvait se réfugier dans les fonds marins ou se cacher sous les sédiments.",
    autres_infos: {
      taille: {
        longueur: '30-60 centimètres',
        hauteur: '15-20 centimètres',
      },
      poids: '1-2 kilogrammes',
    },
    decouverte: {
      date: '1835',
      lieu: 'Écosse',
    },
  },
  {
    nom: 'Cistecephalus',
    etymologie: 'Tête en forme de panier',
    image_url: './assets/images/Cistecephalus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: 'Anomodontia',
      ordre: 'Therapsida',
      sous_ordre: '',
      infra_ordre: 'Dicynodontia',
      micro_ordre: '',
      super_famille: '',
      famille: 'Cistecephalidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Cistecephalus',
      espèces: 'Cistecephalus microrhinus',
    },
    regime_alimentaire: 'Fouisseur',
    habitat: 'Environnements semi-arides, avec des sols sablonneux ou rocailleux',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Permien',
      epoque: 'Lopingien',
      stage: 'Wuchiapingien',
      apparition: 259.8,
      extinction: 254.1,
    },
    description:
      "Cistecephalus avait un crâne large, avec des ouvertures temporales orientées latéralement et son museau était pointu, similaires à celui d'autres fossiles. Il avait néanmoins des orbites relativement grandes, orientés vers l'avant, ce qui suggère une vision binoculaire. Il avait un cou court et les articulations de l'épaule orientées latéralement. Ses membres antérieurs étaient courts et puissants, et ses doigts larges, avec des phalanges fusionnées. Ses membres postérieurs étaient flexibles et servaient probablement à évacuer les débris.",
    autres_infos: {
      taille: {
        longueur: '60 centimètres',
        hauteur: '25 centimètres',
      },
      poids: '10 kilogrammes',
    },
    decouverte: {
      date: '1976',
      lieu: 'Afrique du Sud, Zambie',
    },
  },
  {
    nom: 'Cladoselache',
    etymologie: 'Ressemblant au requin',
    image_url: './assets/images/Cladoselache.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Chondrichthyes',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Cladoselachiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Cladoselachidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Cladoselache',
      espèces:
        'Cladoselache acanthopterygius Dean, Cladoselache brachypterygius Dean, Cladoselache clarki, Cladoselache desmopterygius Dean, Cladoselache fyleri, Cladoselache kepleri, Cladoselache magnificus Claypole, Cladoselache newmani',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Supérieur',
      stage: 'Famennien',
      apparition: -372,
      extinction: -358,
    },
    description:
      "Cladoselache est considéré comme l'un des premiers requins de l'histoire de la Terre. <br /> Ce poisson avait une apparence similaire à celle des requins modernes, avec un corps fusiforme, des nageoires pectorales et pelviennes, ainsi qu'une nageoire caudale hétérocerque (avec une lobe supérieure plus grande que la lobe inférieure). Sa principale caractéristique distinctive était la présence de denticules, des petites écailles dentelées, sur sa peau, similaires à celles des requins d'aujourd'hui. Il se nourrissait probablement de petits poissons et d'autres créatures marines, utilisant sa vitesse et son agilité pour chasser ses proies.",
    autres_infos: {
      taille: {
        longueur: '60 centimètres-2.40 mètres',
        hauteur: '',
      },
      poids: "jusqu'à 200 kilogrammes",
    },
    decouverte: {
      date: '1837',
      lieu: 'Nouvelle-Angleterre',
    },
  },
  {
    nom: 'Conodonta',
    etymologie: 'Dents en forme de cône',
    image_url: './assets/images/Conodontes.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Conodonta',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Conodonta',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: '',
      espèces: '',
    },
    regime_alimentaire: 'Filtreur',
    habitat: '',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Terreneuvien',
      stage: 'Fortunien',
      apparition: -540,
      extinction: -200,
    },
    description:
      "Les conodontes sont des microfossiles marins éteints qui ressemblaient à de petites dents ou à des éléments dentaires. De la taille d'un grain de riz pour les plus gros spécimens, ils se sont éteints au Trias mais sont aujourd'hui considérés comme des indices stratigraphiques précieux pour la datation des roches sédimentaires. Les conodontes étaient des animaux primitifs, sans mâchoires, qui jouaient probablement un rôle dans la capture de proies et la digestion.",
    autres_infos: {
      taille: {
        longueur: "jusqu'à 2 centimètres",
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1850',
      lieu: 'Dans le monde entier',
    },
  },
  {
    nom: 'Crassigyrinus',
    etymologie: '--',
    image_url: './assets/images/Crassigyrinus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Crassigyrinidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Crassigyrinus',
      espèces: 'Crassigyrinus scoticus',
    },
    regime_alimentaire: 'Piscivore',
    habitat: "Marais, étendues d'eau peu profondes",
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Carbonifère',
      epoque: 'Mississippien',
      stage: 'Viséen',
      apparition: -345,
      extinction: -329,
    },
    description:
      "Crassigyrinus était un des premiers amphibiens préhistoriques. Il ressemblait à une sorte de croisement entre un poisson et une salamandre du fait de son corps allongé et serpentiforme. Ses membres étaient courts et robustes, ce qui indique qu'il était mieux adapté à la vie dans l'eau que sur terre. Cet amphibien était équipé de nageoires, ce qui lui permettait de se déplacer avec aisance dans les milieux aquatiques. On pense qu'il passait une partie de sa vie dans l'eau, où il chassait probablement des poissons et d'autres proies aquatiques.",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1831',
      lieu: 'Écosse',
    },
  },
  {
    nom: 'Deinotherium',
    etymologie: 'Animal terrible',
    image_url: './assets/images/Deinotherium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Proboscidea',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Deinotheriidae',
      sous_famille: 'Deinotheriinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Deinotherium',
      espèces: 'D. bozasi, D. giganteum, D. indicum ',
    },
    regime_alimentaire: 'Herbivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Serravallien',
      apparition: -13.82,
      extinction: -3.6,
    },
    description:
      "Deinotherium était un genre d'éléphant préhistorique caractérisé par ses défenses inférieures courbées vers l'avant, contrairement aux éléphants modernes dont les défenses sont incurvées vers le bas. Le Deinotherium avait une trompe allongée et une taille impressionnante. Les plus gros spécimens devaient peser entre 12 et 15 tonnes. C'était l'un des plus grands mammifères terrestres de son époque.",
    autres_infos: {
      taille: {
        longueur: '4 mètres',
        hauteur: '4 mètres',
      },
      poids: '12-15 tonnes',
    },
    decouverte: {
      date: '1883',
      lieu: "Bassin d'Egra, République Tchèque",
    },
  },
  {
    nom: 'Diictodon',
    etymologie: 'Dents en double',
    image_url: './assets/images/Diictodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: 'Anomodontia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Pylaecephalidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Diictodon',
      espèces: 'Diictodon feliceps',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Sols des régions humides au régions sèches',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Permien',
      epoque: 'Guadalupien',
      stage: 'Capitanien',
      apparition: -262,
      extinction: -262,
    },
    description:
      "Diictodon possédait un corps trapu, de courtes pattes et une tête relativement large protégeant un cerveau fort développé, celle-ci se terminant par une sorte de bec corné. Sa caractéristique la plus distinctive était la double rangée de dents dans sa mâchoire inférieure. Ces dents étaient adaptées pour mâcher des plantes. <br /> Diictodon était probablement actif à la fois pendant la journée et la nuit. Il creusait des galeries dans le sol, allant jusqu'à 1.5 mètres de profondeur, ce qui lui permettait de se protéger des prédateurs. Comme beaucoup d'animaux de l'époque, il a finalement disparu à la fin du Permien, peut-être en raison d'événements cataclysmiques comme les éruptions volcaniques massives ou les changements climatiques.",
    autres_infos: {
      taille: {
        longueur: '50 centimètres',
        hauteur: '10-15 centimètres',
      },
      poids: '7-10 kilogrammes',
    },
    decouverte: {
      date: '1876',
      lieu: 'Asie, Afrique du Sud',
    },
  },
  {
    nom: 'Dimétrodon',
    etymologie: 'Dents de deux mètres',
    image_url: './assets/images/Dimetrodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Sphenacodontidae',
      sous_famille: 'Sphenacodontinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Dimetrodon',
      espèces:
        'Dimetrodon angelensis, Dimetrodon borealis, Dimetrodon booneorum, Dimetrodon dollovianus, Dimetrodon gigahomogenes, Dimetrodon grandis, Dimetrodon kempae, Dimetrodon limbatus, Dimetrodon loomisi, Dimetrodon macrospondylus, Dimetrodon milleri, Dimetrodon natalis, Dimetrodon occidentalis, Dimetrodon teutonis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Permien',
      epoque: 'Cisuralien',
      stage: 'Sakmarien',
      apparition: -295,
      extinction: -270,
    },
    description:
      "Cet animal n'est pas un dinosaure, mais un reptile mammalien. Contrairement à son étymologie, ses dents ne mesuraient pas deux mètres, mais étaient de tailles variées, avec de grandes canines pointues et des dents plus petites et tranchantes. <br /> L'une des caractéristiques les plus distinctives du Dimétrodon était la crête osseuse qu'il portait sur son dos. Cette crête, en réalité une série d'extensions vertébrales, pouvait atteindre jusqu'à 1,5 mètre de long. Elle servait probablement à réguler la température corporelle de l'animal, agissant comme un collecteur de chaleur solaire. <br /> C'est l'un des reptiles préhistoriques les plus connus et reconnaissables. Sa crête distinctive et ses dents spécialisées en font une créature fascinante pour les chercheurs et les passionnés de la préhistoire.",
    autres_infos: {
      taille: {
        longueur: '1.70-4.60 mètres',
        hauteur: '2-3 mètres (avec la voile)',
      },
      poids: '300-500 kilogrammes',
    },
    decouverte: {
      date: '1845',
      lieu: 'Île-du-Prince-Édouard, Canada',
    },
  },
  {
    nom: 'Diplocaulus',
    etymologie: 'Double bâton',
    image_url: './assets/images/Diplocaulus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: 'Vertebrata',
      embranchement: 'Chordata',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Amphibia',
      sous_classe: 'Lepospondyli',
      division: '',
      super_ordre: '',
      ordre: 'Nectridea',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Keraterpetontidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Diplocaulus',
      espèces: 'Diplocaulus salamandroides Cope, Diplocaulus magnicornis Cope, Diplocaulus brevirostris',
    },
    regime_alimentaire: 'Carnivore, insectivore, piscivore',
    habitat: "Rivières, étendues d'eau peu profondes",
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Carbonifère',
      epoque: 'Pennsylvanien',
      stage: 'Kasimovien',
      apparition: -306,
      extinction: -255,
    },
    description:
      "Diplocaulus était un amphibien préhistorique qui vivait de la fin du Carbonifère jusqu'à la fin du Permien. Il se distinguait principalement par sa tête unique en forme de fer à cheval, qui ressemblait à deux crânes fusionnés ensemble. Cette tête inhabituelle était en réalité une adaptation pour la natation dans les eaux peu profondes. Ses courbes latérales lui permettaient de glisser efficacement dans l'eau tout en minimisant la résistance. <br /> Malgré son apparence étrange, il était un prédateur efficace et un maillon important de l'écosystème aquatique du Permien.",
    autres_infos: {
      taille: {
        longueur: '0.3-1 mètre',
        hauteur: '15 centimètres',
      },
      poids: '10-20 kilogrammes',
    },
    decouverte: {
      date: '1958',
      lieu: 'États-Unis',
    },
  },
  {
    nom: 'Dipterus',
    etymologie: '--',
    image_url: './assets/images/Dipterus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Dipnoi',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Dipteridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Dipterus',
      espèces:
        ' Dipterus calvini Eastman, Dipterus contraversus Hay, Dipterus costatus Eastman, Dipterus crassus Gross, Dipterus flabelliformis Newberry, Dipterus fleisheri Williams, Dipterus johnsoni Bryant & Johnson, Dipterus macropterus Traquair, Dipterus microsoma Hills, Dipterus minutus Newberry, Dipterus mordax Eastman, Dipterus nelsoni Newberry, Dipterus oervigi Gross, Dipterus serratus Eichwald, Dipterus sherwoodi Newberry, Dipterus uddeni Eastman, Dipterus valenciennesi',
    },
    regime_alimentaire: 'Carnivore',
    habitat: '',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Moyen',
      stage: 'Eifélien',
      apparition: -393,
      extinction: -382,
    },
    description:
      'Dipterus était un poisson prédateur de taille moyenne pour son temps. Il avait un corps allongé et fusiforme, bien adapté à la vie aquatique. Ses nageoires pectorales, ressemblant à des ailes, étaient probablement utilisées pour la propulsion et la stabilisation lors de la nage.',
    autres_infos: {
      taille: {
        longueur: '30-60 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1828',
      lieu: 'Europe, Amérique du Nord',
    },
  },
  {
    nom: 'Doryaspis',
    etymologie: 'Lance et bouclier',
    image_url: './assets/images/Doryaspis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: 'Agnatha',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Pteraspidomorphi',
      sous_classe: 'Heterostraci',
      division: '',
      super_ordre: '',
      ordre: 'Pteraspidiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Protopteraspididae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Doryaspis',
      espèces: 'Doryaspis arctica, Doryaspis groenhorgensis, Doryaspis minor, Doryaspis nathorsti',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Eaux peu profondes des mers et océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Inférieur',
      stage: 'Lochkovien',
      apparition: -419,
      extinction: -410,
    },
    description:
      "Doryaspis appartenait au groupe des ostracodermes, qui étaient des poissons sans mâchoires et dotés de plaques osseuses qui protégeaient leur corps. Mesurant généralement quelques dizaines de centimètres de long. Sa caractéristique la plus distinctive était sa tête en forme de lance avec une grande plaque osseuse en forme de bouclier qui couvrait la partie avant de son corps. Cette plaque offrait une certaine protection contre les prédateurs et les dangers de l'environnement marin. Son absence de mâchoires limitait probablement son régime alimentaire aux particules relativement petites.",
    autres_infos: {
      taille: {
        longueur: '10-20 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1935',
      lieu: 'Norvège',
    },
  },
  {
    nom: 'Drepanaspis',
    etymologie: 'Faucille et bouclier',
    image_url: './assets/images/Drepanaspis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: 'Agnatha',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Pteraspidomorphi',
      sous_classe: 'Heterostraci',
      division: '',
      super_ordre: '',
      ordre: 'Pteraspidiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Psammosteidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Drepanaspis',
      espèces:
        'Drepanaspis gemuendenensis, Drepanaspis sehrieli Gross, Drepanaspis lipperti Gross, Drepanaspis jaegeri Tarlo, Drepanaspis earteri, Drepanaspis edwardsi',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Mers peu profondes',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Inférieur',
      stage: 'Lochkovien',
      apparition: -420,
      extinction: -392,
    },
    description:
      "Drepanaspis possédait un corps allongé et mince, ressemblant à une faucille, d'où son nom. Son corps était recouvert de petites écailles osseuses. <br /> L'une de ses caractéristiques les plus distinctives était sa grande plaque osseuse en forme de bouclier qui couvrait la majeure partie de sa tête, offrant une protection à cette zone vulnérable. Drepanaspis n'était pas un nageur rapide et se nourrissait principalement de petits organismes planctoniques.",
    autres_infos: {
      taille: {
        longueur: '10-30 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1887',
      lieu: 'Europe',
    },
  },
  {
    nom: 'Dunkleosteus',
    etymologie: "En l'honneur du paléontologue David Dunkle, 'osteus' signifiant 'ossature'",
    image_url: './assets/images/Dunkleosteus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: 'Gnathostomata',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Placodermi',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Arthrodira',
      sous_ordre: 'Brachythoraci',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Dunkleosteoidea',
      famille: 'Dunkleosteidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Dunkleosteus',
      espèces: 'Dunkleosteus terrelli',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Supérieur',
      stage: 'Frasnien',
      apparition: -382,
      extinction: -358,
    },
    description:
      "Dunkleosteus était un gigantesque poisson, prédateur redoutable des mers anciennes. Il avait un corps massif et aplati, recouvert d'une armure osseuse épaisse plutôt que d'écailles. Son crâne était impressionnant, avec des mâchoires larges et puissantes remplies de dents acérées, qui pouvaient broyer les coquilles et les os de ses proies; a force de morsure était de 6 000 N à la pointe et de 7 400 N sur les bords. <br /> Dunkleosteus a prospéré dans les eaux chaudes et peu profondes des océans de la fin du Dévonien, et bien que plus petit que d'autres, il est considéré comme l'un des plus grands prédateurs marins de tous les temps de par sa dangerosité.",
    autres_infos: {
      taille: {
        longueur: '4 mètres',
        hauteur: '1 mètres',
      },
      poids: '1.2 tonne',
    },
    decouverte: {
      date: '1867',
      lieu: 'Lac Erie, Ohio, États-Unis',
    },
  },
  {
    nom: 'Edaphosaurus',
    etymologie: '--',
    image_url: './assets/images/Edaphosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: 'Tetrapoda',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Edaphosaurus',
      espèces:
        'Edaphosaurus boanerges, Edaphosaurus colohistion, Edaphosaurus cruciger, Edaphosaurus novomexicanus, Edaphosaurus pogonias',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Carbonifère',
      epoque: 'Pennsylvanien',
      stage: 'Gzhélien',
      apparition: -303,
      extinction: -272,
    },
    description:
      "Tout comme Dimetrodon, Édaphosaurus était doté d'une « voile » dorsale cutanée et vascularisée lui servant sans doute à réguler sa température. Il ne possédait pas de système digestif efficace. En effet, il faut pour cela que la température interne du corps soit relativement élevée: la voile devait permettre de pallier cet inconvénient en élevant artificiellement la température corporelle de l'animal.",
    autres_infos: {
      taille: {
        longueur: '0.5-3.50 mètres',
        hauteur: '',
      },
      poids: "jusqu'à 300 kilogrammes",
    },
    decouverte: {
      date: '1882',
      lieu: 'Amérique du Nord',
    },
  },
  {
    nom: 'Haikouichthys',
    etymologie: 'Poisson de Haikou',
    image_url: './assets/images/Haikouichthys.png',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: 'Eumetazoa',
      rameau: '',
      infra_règne: '',
      super_embranchement: 'Deuterostomia',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: 'Agnatha',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Myllokunmingiida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Myllokunmingiidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Haikouichthys',
      espèces: 'Haikouichthys ercaicunensis',
    },

    regime_alimentaire: 'Charognard',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 3',
      apparition: -518,
      extinction: -518,
    },
    description:
      "Haikouichthys était caractérisé par un corps allongé et mince, dépourvu de mâchoires et d'écailles. Haikouichthys possédait une fente branchiale unique de chaque côté de son corps, ce qui suggère qu'il respirait principalement par branchies. Il est considéré comme l'un des premiers vertébrés connus. <br /> Les fossiles d'Haikouichthys sont si bien conservés qu'il est possible de distinguer certains de ses organes internes.",
    autres_infos: {
      taille: {
        longueur: '2-3 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1999',
      lieu: 'Chine',
    },
  },
  {
    nom: 'Halkieria',
    etymologie: 'Du nom de Frans Halkeri, son découvreur',
    image_url: './assets/images/Halkieria.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: 'Lophotrochozoa',
      embranchement: 'Mollusca',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Halkieriidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Halkieria',
      espèces:
        'Halkieria alata, Halkieria amorpha, Halkieria bisulcata, Halkieria costulata, Halkieria curvativa, Halkieria deplanatiformis, Halkieria desquamata,Halkieria directa, Halkieria elonga, Halkieria equilateralis, Halkieria folliformis, Halkieria fordi, Halkieria hexagona, Halkieria lata, Halkieria longa, Halkieria longispinosa, Halkieria maidipingensis, Halkieria mina, Halkieria mira, Halkieria obliqua, Halkieria operculus, Halkieria pennata, Halkieria phylloidea, Halkieria praeinguis, Halkieria projecta Bokova, Halkieria sacciformis Meshkova, Halkieria solida, Halkieria sthenobasis, Halkieria stonei, Halkieria symmetrica, Halkieria terastios, Halkieria uncostata, Halkieria undulata, Halkieria ventricosa, Halkieria wangi, Halkieria zapfei ',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Terreneuvien',
      stage: 'Fortunien',
      apparition: -530,
      extinction: -505,
    },
    description:
      "Halkieria appartenait au groupe des animaux à corps mou et possédait une coquille en forme de bouclier. Halkieria avait une symétrie bilatérale et se déplaçait en utilisant un pied musclé. Il possédait également une bouche, des organes digestifs et un système nerveux primitif. Bien que petit, Halkieria occupait un rôle important dans les écosystèmes marins de l'époque.",
    autres_infos: {
      taille: {
        longueur: '10-15 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1967',
      lieu: 'Partout autour du globe',
    },
  },
  {
    nom: 'Hallucigenia',
    etymologie:
      "Le paléontologue Simon Conway Morris choisit en 1977 le nom de 'Hallucigenia' à cause des « qualités étranges et fantaisistes semblant sorties d'un rêve » de l'animal (comme une hallucination)",
    image_url: './assets/images/Hallucigenia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Lobopodia',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Hallucigeniidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Hallucigenia',
      espèces: 'Hallucigenia sparsa, Hallucigenia fortis Hou & Bergström, Hallucigenia hongmeia',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 3',
      apparition: -521,
      extinction: -497,
    },
    description:
      "Cet animal marin à la silouhette atypique avait un corps allongé, sept paires de tentacules se terminant par des « pinces », alignées d'un côté et sept paires d'épines jointes de l'autre. Six tentacules plus petits peuvent être configurés en trois paires derrière les sept plus gros. Il y a enfin une extension tubulaire molle du corps derrière les tentacules.. Ses caractéristiques étranges et inhabituelles en ont fait un fossile fascinant pour les scientifiques.",
    autres_infos: {
      taille: {
        longueur: '0.5-5 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1909',
      lieu: 'Colombie-Britanique',
    },
  },
  {
    nom: 'Hyolitha',
    etymologie: '--',
    image_url: './assets/images/Hyolitha.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: 'Lophotrochozoa',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Hyolitha',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: '',
      espèces: '',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Terreneuvien',
      stage: 'Fortunien',
      apparition: -540,
      extinction: -252,
    },
    description:
      "Les Hyolitha sont un groupe éteint d'animaux marins qui ressemblaient à des coquillages en forme de cônes avec une coquille en forme de tube. On pense qu'ils étaient fixés au fond de l'océan par une partie de leur coquille, tandis que leur corps se trouvait à l'intérieur du tube. Les Hyolitha étaient de petites créatures et leur mode de vie exact reste encore en grande partie mystérieux.",
    autres_infos: {
      taille: {
        longueur: '3 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1963',
      lieu: '',
    },
  },
  {
    nom: 'Marrella',
    etymologie: "Du nom d'un ami de son découvreur (Charles Doolittle Walcott): Dr John Edmund Marr",
    image_url: './assets/images/Marrella.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Marrellomorpha',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Marrellida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Marrellidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Marrella',
      espèces: 'Marrella splendens',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Mers et océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Miaolingien',
      stage: 'Guzhangien',
      apparition: -500,
      extinction: -470,
    },
    description:
      "Cette petite créature marine possédait un corps allongé et divisé en segments, avec de multiples paires de pattes fines et ramifiées. Sa tête était munie d'antennes et de mandibules. On pense que Marrella était un animal filtreur se nourrissant de particules alimentaires présentes dans l'eau. Ses fossiles fournissent des informations précieuses sur la biodiversité au cours de cette période ancienne de l'histoire de la Terre.",
    autres_infos: {
      taille: {
        longueur: '5 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1909',
      lieu: 'Colombie-Britanique',
    },
  },
  {
    nom: 'Méganeura',
    etymologie: 'Ailes géantes',
    image_url: './assets/images/Meganeura.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Insecta',
      sous_classe: '',
      division: 'Palaeoptera',
      super_ordre: 'Odonatoptera',
      ordre: 'Meganisoptera',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Meganeuridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Meganeura',
      espèces: 'Meganeura brongniarti, Meganeura monyi, Meganeura vischerae',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Marais forestiers',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Carbonifère',
      epoque: 'Pennsylvanien',
      stage: 'Kasimovien',
      apparition: -305,
      extinction: -299,
    },
    description:
      "Cet insecte est l'un des plus énigmatique du Carbonifère. Il s'agit d'une libellule géante caractérisée par l'envergure exceptionnelle de ses ailes. Il y a fort à parier que l'on pouvait l'entendre venir de loin. Chassant dans les endroits forestiers et à proximité de plans d'eau, il se nourissait de petit lézards existants à cette époque. C'est la deuxième plus grande libellule de tous les temps.",
    autres_infos: {
      taille: {
        longueur: '30 centimètres',
        hauteur: '',
        envergure: '70 centimètres',
      },
      poids: '150 grammes',
    },
    decouverte: {
      date: '1880',
      lieu: 'Auvergne, France',
    },
  },
  {
    nom: 'Nectocaris',
    etymologie: 'Crevette nageuse',
    image_url: './assets/images/Nectocaris.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Mollusca',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Cephalopoda',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Nectocarididae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Nectocaris',
      espèces: 'Nectocaris pteryx',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 3',
      apparition: -521,
      extinction: -505,
    },
    description:
      "Cette créature de petite taille avec un corps allongé et divisé en segments, possédait de nombreux appendices, y compris des nageoires latérales et des tentacules sur sa tête, qui étaient probablement utilisés pour capturer sa nourriture. Nectocaris est considéré comme l'un des premiers exemples connus de céphalopodes, une famille d'animaux marins qui comprend les poulpes, les calmars et les seiches. Son fossile a été découvert dans des gisements en Australie et au Canada, fournissant des informations précieuses sur la diversité des formes de vie au cours du Cambrien.",
    autres_infos: {
      taille: {
        longueur: '10 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '',
      lieu: 'Colombie-Britanique',
    },
  },
  {
    nom: 'Odontogriphus',
    etymologie: 'Enigme dentée',
    image_url: './assets/images/Odontogriphus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: 'Lophotrochozoa',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Odontogriphidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Odontogriphus',
      espèces: 'Odontogriphus omalus',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Miaolingien',
      stage: 'Wuliuen',
      apparition: -509,
      extinction: -504,
    },
    description:
      "Odontogriphus avait un corps allongé en forme de ver, mesurant jusqu'à 12 centimètres de long. Il possédait une paire de « palpes » (probablement des organes sensoriels) figurant sur les bords de l'extrémité frontale. La bouche en forme de « U » est légèrement creuse et entourée de petites structures coniques pointues, il s'agirait de petits tentacules constituant un système de collecte de nourriture.",
    autres_infos: {
      taille: {
        longueur: '6-12 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1910',
      lieu: 'Colombie-Britanique',
    },
  },
  {
    nom: 'Opabinia',
    etymologie: 'Dérivant du lieu de sa découverte: le Mont Opabin (Canada)',
    image_url: './assets/images/Opabinia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Dinocaridida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Opabiniidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Opabinia',
      espèces: 'Opabinia regalis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Miaolingien',
      stage: 'Wuliuen',
      apparition: -509,
      extinction: -509,
    },
    description:
      "Opabinia avait un corps allongé avec une tête en forme de trompe qui possédait deux grands yeux composés et une bouche dirigée vers le bas. Sur sa tête, il y avait également cinq yeux plus petits. Il possédait de nombreux membres minces et articulés, dont un pair de pinces à l'avant. Opabinia était dotée d'un appendice en forme de queue qui ressemblait à une plume et qui était probablement utilisé pour se propulser dans l'eau. Cette créature énigmatique est considérée comme l'un des premiers exemples connus de formes de vie complexe de l'ère cambrienne.",
    autres_infos: {
      taille: {
        longueur: '4-7 centimètres',
        hauteur: '1911',
      },
      poids: '',
    },
    decouverte: {
      date: '',
      lieu: 'Colombie-Britanique',
    },
  },
  {
    nom: 'Pikaia',
    etymologie: 'Cordon amer',
    image_url: './assets/images/Pikaia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: 'Eumetazoa',
      rameau: '',
      infra_règne: '',
      super_embranchement: 'Deuterostomia',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Pikaiidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: '',
      espèces: 'Pikaia gracilens',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Mers peu profondes',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 4',
      apparition: -513,
      extinction: -505,
    },
    description:
      "Pikaia était un animal segmenté en forme de ver possédant un corps allongé avec une symétrie bilatérale et était pourvu d'une queue en forme de nageoire. Il n'avait pas de véritable colonne vertébrale, mais possédait une structure semblable à une tige appelée 'notochorde', qui est considérée comme un précurseur de la colonne vertébrale. Pikaia se nourrissait probablement de particules organiques en suspension dans l'eau.",
    autres_infos: {
      taille: {
        longueur: '5-10 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1911',
      lieu: 'Colombie-Britanique',
    },
  },
  {
    nom: 'Pterygotus',
    etymologie: "L'aile",
    image_url: './assets/images/Pterygotus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: 'Chelicerata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Eurypterida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Pterygotioidea',
      famille: 'Pterygotidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Pterygotus',
      espèces:
        'Pterygotus anglicus, Pterygotus arcuatu, Pterygotus barrandei, Pterygotus bolivianus, Pterygotus carman, Pterygotus cobbi, Pterygotus denticulatus, Pterygotus floridanu, Pterygotus gaspesiensis, Pterygotus grandidentatu, Pterygotus impacatus, Pterygotus kopaninens, Pterygotus lanarkensi, Pterygotus lightbody, Pterygotus ludens, Pterygotus marylandicus, Pterygotus monroensi',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Mers peu profondes',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Silurien',
      epoque: 'Wenlock',
      stage: 'Sheinwoodien',
      apparition: -428,
      extinction: -372,
    },
    description:
      "Pterygotus est le plus gros scorpion de tous les temps. Les plus gros specimens mesuraient près de 3 mètres de long. Du fait de sa carapce solide, de ces pinces tranchantes et de son corps puissant, il était un prédateur efficace. Il possédait une paire d'yeux composés gigantesques, et également une autre paire d'yeux plus petits au centre de sa tête et quatre paires de pattes pour se déplacer, une cinquième paire, modifiée en sortes de palmes, ainsi qu'une paire de grandes pinces, lui permettant de capturer des proies. Six tergites principaux, ou sections de queue, contenaient les branchies et les organes reproducteurs de l'animal. <br /> La très grande paire d'yeux de Pterygotus suggère fortement que c'était un prédateur qui s'orientait grâce à sa vue. Il utilisait ses palmes pour nager et pouvait probablement accélérer en utilisant sa queue comme une troisième palme.",
    autres_infos: {
      taille: {
        longueur: '1.75-3 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1839',
      lieu: 'Écosse',
    },
  },

  {
    nom: 'Sidneyia',
    etymologie: 'Walcott le baptisa du prénom de son fils, Sidney',
    image_url: './assets/images/Sidneyia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Sidneyia',
      espèces: 'Sidneyia inexpectans Walcott, Sidneyia minor, Sidneyia malongensis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: '',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 3',
      apparition: -521,
      extinction: -509,
    },
    description:
      "Sidneyia possède un corps allongé et segmenté, recouvert d'une carapace dure, et il est considéré comme l'un des premiers prédateurs connus. Le contenu de l’intestin indique que le régime de Sidneyia se composait principalement de petits trilobites et hyolithes (mollusques). Sidneyia avait une paire d'antennes à l'avant de sa tête, suivie de plusieurs paires de pattes.",
    autres_infos: {
      taille: {
        longueur: '5-13 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1911',
      lieu: 'Colombie-Britanique',
    },
  },
  {
    nom: 'Wiwaxia',
    etymologie: "De la langue Pied-Noir signifiant 'Petit éclat de graisse' ou 'Ver de suif'",
    image_url: './assets/images/Wiwaxia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: 'Lophotrochozoa',
      embranchement: 'Mollusca',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Wiwaxiidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Wiwaxia',
      espèces: 'Wiwaxia corrugata, Wiwaxia foliosa, Wiwaxia papilio, Wiwaxia taijiangensis, Wiwaxia herka',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 3',
      apparition: -520,
      extinction: -505,
    },
    description:
      "Wiwaxia a de longues épines qui se projettent en deux rangées le long du dos, apportant à la créature une protection contre ses prédateurs. Le reste de la partie supérieure du dos est couvert de petites plaques dures qui se chevauchent nommées 'sclérites'. Chaque sclérite est rattachée au corps par une base similaire à une racine et il est supposé que l'animal grossissait avec des mues de ces plaques de temps à autre. ",
    autres_infos: {
      taille: {
        longueur: '5 centimètres',
        hauteur: '1 centimètres (sans les épines)',
      },
      poids: '',
    },
    decouverte: {
      date: '1899',
      lieu: 'Colombie-Britanique',
    },
  },

  {
    nom: 'Placerias',
    etymologie: 'Corps large',
    image_url: './assets/images/Placerias.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Synapsida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Therapsida',
      sous_ordre: 'Anomodontia',
      infra_ordre: 'Dicynodontia',
      micro_ordre: '',
      super_famille: '',
      famille: 'Stahleckeriidae',
      sous_famille: 'Placeriinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Placerias',
      espèces: 'Placerias hesternus',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme, marais peu profonds',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Trias',
      epoque: 'Supérieur',
      stage: 'Carnien',
      apparition: -231,
      extinction: -216,
    },
    description:
      "Placerias figure parmi l'un des plus grands herbivores du Trias supérieur. Son crâne mesurait 68 centimètres de long et disposait d'un cou puissant, des jambes robustes et un corps en forme de tonneau. Il devait passer une grande partie de son temps durant les saisons des pluies à se tremper dans les fleuves et à mâcher la végétation des berges. Rester dans l'eau aurait également donné à Placerias une certaine protection contre les prédateurs terrestres tels que Postosuchus. <br /> Placerias aurait utilisé son bec pour couper à travers des branches et des racines épaisses avec deux défenses courtes qui pourraient être utilisées pour se protéger des prédateurs",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '1.30 mètre',
      },
      poids: '1 tonne',
    },
    decouverte: {
      date: '1930',
      lieu: 'Arizona',
    },
  },
  {
    nom: 'Stethacanthus',
    etymologie: 'Épine de poitrine',
    image_url: './assets/images/Stethacanthus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Chondrichthyes',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Symmoriiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Stethacanthidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Stethacanthus',
      espèces:
        'Stethacanthus altonensis, Stethacanthus concavus, Stethacanthus gansuensis, Stethacanthus neilsoni, Stethacanthus praecursor, Stethacanthus resistens, Stethacanthus thomasi, Stethacanthus productus,',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Supérieur',
      stage: 'Frasnien',
      apparition: -382.7,
      extinction: -298.9,
    },
    description:
      "Le mâle du genre Stethacanthus était doté d'une nageoire dorsale en forme d'enclume; la femelle n'en possédait certainement pas. Stethacanthus possédait de nombreux denticules dermiques au sommet du crâne, les scientifiques pensent qu'ils servaient à repousser d'éventuels prédateurs. Stethacanthus a, à la base des nageoires pectorales, des fouets pectoraux.",
    autres_infos: {
      taille: {
        longueur: '0.70-2 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: 'Fin des années 1800, mais formellement identifié en 1974',
      lieu: 'Montana',
    },
  },
  {
    nom: 'Basilosaurus',
    etymologie: 'Lézard royal',
    image_url: './assets/images/Basilosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Cetartiodactyla',
      sous_ordre: '',
      infra_ordre: 'Cetacea',
      micro_ordre: '',
      super_famille: '',
      famille: 'Basilosauridae',
      sous_famille: 'Basilosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Basilosaurus',
      espèces: 'Basilosaurus cetoides, Basilosaurus isis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Paléogène',
      epoque: 'Éocène',
      stage: 'Bartonien',
      apparition: -41.2,
      extinction: -33.9,
    },
    description:
      "Cette baleine carnivore préhistorique était le plus grand prédateur marin de son temps. Sa denture était taillée pour chasser des poissons et des dauphins préhistoriques. L'animal possédait deux petites nageoires postérieures qui devaient participer principalement à l'accouplement.",
    autres_infos: {
      taille: {
        longueur: '15-20 mètres',
        hauteur: '',
      },
      poids: '60 tonnes',
    },
    decouverte: {
      date: 'XiXè siècle',
      lieu: 'Alabama',
    },
  },
  {
    nom: 'Postosuchus',
    etymologie: 'Crocodile de Post',
    image_url: './assets/images/Postosuchus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Rauisuchia',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: 'Rauisuchidae',
      tribu: '',
      sous_tribu: '',
      genre: 'Postosuchus',
      espèces: 'Postosuchus kirkpatricki, Postosuchus alisonae',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Trias',
      epoque: 'Supérieur',
      stage: 'Norien',
      apparition: -227,
      extinction: -208.5,
    },
    description:
      "Postosuchus était le plus grand prédateur de son temps. Contrairement à son apparence, il ne s'agit pas d'un dinosaure. Il appartient à un groupe de reptil plus ancien. Cela dit, Postosuchus était contemporains des premiers dinosaures. <br /> Son crâne mesurait 55 cm de long et 21 cm de large et profond. Ses dents étaient en forme de dague et pouvaient mesurer près de 7 centimètres. Son cou était long, son torse court et sa queue longue. Outre les restes du squelette, les paléontologues identifient également les ostéodermes, qui sont des plaques épaisses formant des écailles. Celles-ci étaient sur le dos, le cou et éventuellement au-dessus ou sous la queue. <br /> Il devait se déplacer à quatre pattes, mais à l'examen des os de son bassin, les paléontologues trouvent raisonnable de penser que Postosuchus pouvait également choisir la posture bipède.",
    autres_infos: {
      taille: {
        longueur: '5 mètres',
        hauteur: '1.20 mètres (à quatre pattes)',
      },
      poids: '300 kilogrammes',
    },
    decouverte: {
      date: '1980',
      lieu: 'Texas',
    },
  },
  {
    nom: 'Pampaphoneus',
    etymologie: 'Tueur de la Pampa',
    image_url: './assets/images/Pampaphoneus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Synapsida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: 'Dinocephalia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Anteosauridae',
      sous_famille: 'Syodontinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Pampaphoneus',
      espèces: 'Pampaphoneus biccai',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Pampa argentine',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Permien',
      epoque: 'Guadalupien',
      stage: 'Wordien',
      apparition: -268,
      extinction: -265,
    },
    description:
      "Ce carnivore impitoyable chassait dans ce qui est aujourd'hui l'Amérique du Sud. Son crâne mesurait 32 centimètres et ses canines mesuraient 7 centimètres. <br /> C’est le plus grand animal prédateur connu jusqu’à ce jour en Amérique du Sud.",
    autres_infos: {
      taille: {
        longueur: '3 mètres',
        hauteur: '1 mètre',
      },
      poids: '400 kilogrammes',
    },
    decouverte: {
      date: '2012',
      lieu: 'Brésil',
    },
  },
  {
    nom: 'Inostrancevia',
    etymologie: "Nommé en l'honneur du géologue Alexandre Inostrantsev",
    image_url: './assets/images/Inostrancevia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Synapsida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Therapsida',
      sous_ordre: 'Gorgonopsia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Gorgonopsidae',
      sous_famille: 'Inostranceviinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Inostrancevia',
      espèces: 'Inostrancevia africana, Inostrancevia alexandri, Inostrancevia latifrons, Inostrancevia uralensis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Permien',
      epoque: 'Lopingien',
      stage: 'Wuchiapingien',
      apparition: -259,
      extinction: -252.1,
    },
    description:
      "L'une des caractéristiques les plus reconnaissables d'Inostrancevia, et des autres gorgonopsiens en général, est la présence de longues canines ressemblant à des sabres sur la mâchoire supérieure et inférieure. C'était le plus gros prédateur de son temps. Les récentes études montrent qu'il possédait une force de morsure exceptionnelle, mais assez forte pour casser les os. Son crâne pouvant mesurer jusqu'à 50 centimètres permettait une ouverture massive, permettant à ce dernier de délivrer une morsure fatale à sa proie.",
    autres_infos: {
      taille: {
        longueur: '3 mètres',
        hauteur: '1.60 mètres',
      },
      poids: '300 kilogrammes',
    },
    decouverte: {
      date: '1890',
      lieu: "Russie d'Europe",
    },
  },
  {
    nom: 'Pterodaustro',
    etymologie: 'Aile du Sud',
    image_url: './assets/images/Pterodaustro.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Pterosauria',
      sous_ordre: 'Pterodactyloidea',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Ctenochasmatidae',
      sous_famille: 'Ctenochasmatinae',
      tribu: 'Pterodaustrini',
      sous_tribu: '',
      genre: 'Pterodaustro',
      espèces: 'Pterodaustro guinazui',
    },
    regime_alimentaire: 'Carnivore filtreur',
    habitat: 'Airs, terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Aptien',
      apparition: -125,
      extinction: -100,
    },
    description:
      "Cet animal préhistorique avait de petits membres postérieurs qui se terminaient par des pattes recouvertes d'une peau épaisse et squameuse, avec de longs doigts et des griffes acérées et incurvées. <br /> Il avait probablement l'habitude de se tenir debout avec ses pieds dans des eaux peu profondes et de filtrer de très petites créatures aquatiques avec son bec. <br /> L'élément particulier le plus intéressant de cette créature était le bec recourbé vers le haut, qui avait un rôle filtrant et qui impressionnait par son unicité, par sa construction spéciale. <br /> La partie inférieure du bec était bordée à l'intérieur d'environ 500 poils acérés ressemblant à des dents symétriques et arrondies, à l'aide desquels l'animal a soudainement attrapé et a réussi à retenir de petits animaux aquatiques de l'eau de mer. La partie supérieure du bec était munie de nombreuses petites dents acérées qui aidaient au déchiqustage ultérieur de la proie.",
    autres_infos: {
      taille: {
        longueur: '',
        hauteur: '50 centimètres',
        envergure: '1.30-3 mètres',
      },
      poids: '8-10 kilogrammes',
    },
    decouverte: {
      date: '1963',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Ornitholestes',
    etymologie: "Voleur d'oiseeaux",
    image_url: './assets/images/Ornitholestes.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: 'Coelurosauria',
      micro_ordre: '',
      super_famille: 'Coeluroidea',
      famille: 'Coeluridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Ornitholestes',
      espèces: 'Ornitholestes hermanni',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme, forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Supérieur',
      stage: 'Kimméridgien',
      apparition: -154,
      extinction: -152,
    },
    description:
      "Ce dinosaure bipède possédait une tête proportionnellement plus petite que celle de la plupart des autres dinosaures prédateurs, mais le crâne était fortement construit, avec un museau court et une mâchoire inférieure robuste. Les orbites (orbites oculaires) étaient assez grandes, mesurant plus de 25 % de la longueur du crâne. <br /> Il n'y a aucune indication d'un anneau oculaire osseux. Son cou était court avec une légère forme de S. Sa queue ressemblait à un fouet. Son squelette était constitué d'os très légers.",
    autres_infos: {
      taille: {
        longueur: '2-3 mètres',
        hauteur: '1.50 mètres',
      },
      poids: '15-20 kilogrammes',
    },
    decouverte: {
      date: '1900',
      lieu: 'Wyoming',
    },
  },
  {
    nom: 'Daeodon',
    etymologie: 'Dent destructrice',
    image_url: './assets/images/Daeodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Eutheria',
      division: '',
      super_ordre: 'Laurasiatheria',
      ordre: 'Cetartiodactyla',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Entelodontidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Daeodon',
      espèces: 'Daeodon shoshonensis',
    },
    regime_alimentaire: 'Omnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Paléogène',
      epoque: 'Oligocène',
      stage: 'Rupélien',
      apparition: -29,
      extinction: -15.97,
    },
    description:
      "Ce monstre de cauchemar est l'un des prédateurs les plus impitoyables que la Terre ait portée. Cet animal, ressemblant à un phacochère, portait des excroissances sur la tête. Celle-ci était proportionnellement plus courte, plus lourde, plus robuste et plus puissante que chez les autres entélodontes. Ses pattes lui permettaient de courir vite (60 km/h) mais sur de courtes distances : chaque sabot avait 2 doigts. <br /> Il n’est pas exclu que Daeodon ait pu faire fuir des prédateurs comme Hyaenodon ou certaines espèces de tigres aux dents de sabre pour s’emparer de leurs proies. <br /> Daeodon pouvait aussi piller des nids de reptiles ou d’oiseaux de grande taille, ou encore broyer des racines, des tubercules, des noix de coco, des plantes grâce à sa robuste dentition.",
    autres_infos: {
      taille: {
        longueur: '4 mètres',
        hauteur: '2.20 mètres',
      },
      poids: '1.5 tonne',
    },
    decouverte: {
      date: '1878',
      lieu: 'Amérique du Nord',
    },
  },
  {
    nom: 'Kelenken',
    etymologie: "D'un démon de la mythologie du peuple Tehuelche",
    image_url: './assets/images/Kelenken.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Aves',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Cariamiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Phorusrhacidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Kelenken',
      espèces: 'Kelenken guillermoi',
    },
    regime_alimentaire: 'Carnivore, charognard',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Langhien',
      apparition: -15,
      extinction: -14,
    },
    description:
      "Cet oiseau non-volant était l'un des plus grands que la Terre ait porté. <br /> Sa tête faisait plus de 70 centimètres de long, bec compris. Il mangeait des rongeurs, des reptiles et de petits mammifères. Son crâne très rigide et raide indique qu'il pouvait avaler de petites proies entières ou cibler des proies plus grosses en leur donnant des coups de bec répétitifs de manière à les assommer et/ou à sectionner la moelle épinière. <br /> L'analyse de son crâne permet de comprendre qu'il était incapable d'arracher les morceaux de viande de ces proies en bougeant sa tête de gauche à droite.",
    autres_infos: {
      taille: {
        longueur: '',
        hauteur: '3 mètres',
      },
      poids: '100 kilogrammes',
    },
    decouverte: {
      date: '1999',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Coelophysis',
    etymologie: 'Forme creuse',
    image_url: './assets/images/Coelophysis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Coelophysoidea',
      famille: 'Coelophysidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Coelophysis',
      espèces: 'Coelophysis bauri, Coelophysis rhodesiensis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Trias',
      epoque: 'Supérieur',
      stage: 'Carnien',
      apparition: -237,
      extinction: -201.3,
    },
    description:
      "Ce dinosaure devait être rapide et agile. Sa gueule étroite et son cou souple lui permettait de saisir de petites proies rapides. Les dents sont incurvées vers l'arrière et la mâchoire était faite de manière que les deux parties puissent glisser l'une sur l'autre. Cœlophysis chassait très probablement en meute. Il pouvait être cannibale.",
    autres_infos: {
      taille: {
        longueur: '3 mètres',
        hauteur: '1 mètre',
      },
      poids: '15-20 kilogrammes',
    },
    decouverte: {
      date: '1841',
      lieu: 'Nouveau-Mexique',
    },
  },
  {
    nom: 'Argentavis',
    etymologie: 'Oiseau argentin',
    image_url: './assets/images/Argentavis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Aves',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Cathartiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Teratornithidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Argentavis',
      espèces: 'Argentavis magnificens',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Pampa argentine',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Tortonien',
      apparition: -9,
      extinction: -6.8,
    },
    description:
      "Cet oiseau ressemblait à un condor gigantesque, se déplaçant comme ce dernier essentiellement en planant car incapable de battre des ailes durant une longue période du fait d'une musculature de poitrine relativement peu développée, choisissant des points élevés pour faciliter son envol. Il devait pondre un ou deux œufs d'une masse d'un peu plus d'un kilo chacun (plus petit qu'un œuf d'autruche) tous les deux ans. <br /> Les considérations climatiques rendent possible l'incubation des œufs pendant l'hiver, les parents partageant les tâches de couvade et d'approvisionnement en nourriture tous les quelques jours. Il est aussi probable que les jeunes aient été indépendants après seize mois bien que pas complètement mûrs, n'arrivant à maturité qu'à douze ans environ. La mortalité devait aussi être très faible. En effet, pour maintenir une population viable, il faut que moins de 2 % des oiseaux meurent chaque année. <br /> Argentavis n'a subi pratiquement aucune prédation, et les oiseaux mourait principalement de vieillesse, ou à cause d'accident ou de maladie.",
    autres_infos: {
      taille: {
        longueur: '3.5 mètres',
        hauteur: '1.70-2 mètres',
        envergure: '7 mètres',
      },
      poids: '60-110 kilogrammes',
    },
    decouverte: {
      date: '1980',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Anancus',
    etymologie: '--',
    image_url: './assets/images/Anancus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Eutheria',
      division: '',
      super_ordre: '',
      ordre: 'Proboscidea',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Elephantoidea',
      famille: 'Gomphotheriidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Anancus',
      espèces:
        'Anancus alexeevae, Anancus arvernensis, Anancus capensis, Anancus cuneatus, Anancus kazachstanensis, Anancus kenyensis, Anancus lehmanni, Anancus osiris, Anancus perimensi, Anancus sinensis, Anancus sivalensis, Anancus ultimus',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme, forêts',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Quaternaire',
      epoque: 'Pliocène',
      stage: 'Plaisancien',
      apparition: -3.6,
      extinction: -1,
    },
    description:
      "Cet animal ressemblait beaucoup aux éléphants d'aujourd'hui, à ceci près que ses défense pouvaient mesurer jusqu'à 4 mètres de long. <br /> Anancus utilisait certainement ses défenses comme armes. Ses molaires ressemblaient plus à celles du porc et du tapir qu’à celles des éléphants actuels, n’étant pas composées de lames. Anancus avait en outre une dentition différente et de plus petites pattes que l’éléphant actuel. Il se nourrissait d’arbres, d’arbustes, de tubercules ou de racines.",
    autres_infos: {
      taille: {
        longueur: '4 mètres (défenses non-comprises)',
        hauteur: '3 mètres',
      },
      poids: '5-6 tonnes',
    },
    decouverte: {
      date: '1855',
      lieu: '',
    },
  },
  {
    nom: 'Muttaburrasaurus',
    etymologie: 'Fait référence à la ville de Muttaburra près de laquelle il a été découvert',
    image_url: './assets/images/Muttaburrasaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Ornithischia',
      sous_ordre: 'Ornithopoda',
      infra_ordre: 'Iguanodontia',
      micro_ordre: '',
      super_famille: '',
      famille: 'Rhabdodontidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Muttaburrasaurus',
      espèces: 'Muttaburrasaurus langdoni',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Albien',
      apparition: -112,
      extinction: -105,
    },
    description:
      "Muttaburrasaurus était aussi bien capable de se déplacer à deux ou à quatre pattes. Les trois doigts du milieu de ses pattes avant étaient joints et formaient un support en forme de crochet pour marcher. Sa mâchoire puissante pouvait certainement l'aider à mâcher. <br /> Du fait de sa cavité nasale particulière, il est possible que Muttaburrasaurus pouvait produire des sons puissants, mais les paléontologues n'ont pas retrouvé de tissu nasal fossillisé.",
    autres_infos: {
      taille: {
        longueur: '9 mètres',
        hauteur: '2-2.50 mètres',
      },
      poids: '2-3 tonnes',
    },
    decouverte: {
      date: '1963',
      lieu: 'Australie',
    },
  },
  {
    nom: 'Mégalodon',
    etymologie: 'Grande dent',
    image_url: './assets/images/Megalodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Chondrichthyes',
      sous_classe: '',
      division: '',
      super_ordre: 'Selachimorpha',
      ordre: 'Lamniformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Otodontidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Otodus',
      espèces: 'Otodus megalodon',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Aquitanien',
      apparition: -23,
      extinction: -3.6,
    },
    description:
      "Le requin Mégalodon est connu pour être l'un des animaux carnivores les plus célèbres de l'Histoire. <br /> Bien que considéré comme l'un des prédateurs les plus grands et les plus puissants à avoir jamais vécu, le mégalodon n'est connu que par des restes fragmentaires, et son apparence et sa taille maximale demeurent incertaines. Mais des calculs permettent de se faire une idée de la taille de l'animal qui reste toutefois impressionnante. Ses dents de 15 centimètres épaisses et robustes étaient conçues pour attrapper ses proies et de leur infliger des blessures mortelles. La mâchoire de Mégalodon est suffisement puissante pour broyer les os. <br /> Les premières descriptions de Mégalodon remontent à la Renaissance, où, sur la base de dents, les gens croyaient qu'il s'agissait de dents de dragon ou de serpent, mais c'est en 1667 que le naturaliste danois Nicolas Sténon a compris qu'il s'agissait de dents de requin.",
    autres_infos: {
      taille: {
        longueur: '10-20 mètres',
        hauteur: '',
      },
      poids: '60-100 tonnes',
    },
    decouverte: {
      date: 'Renaissance',
      lieu: 'Autour du globe',
    },
  },
  {
    nom: 'Titanoboa',
    etymologie: 'Boa titanesque',
    image_url: './assets/images/Titanoboa.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Squamata',
      sous_ordre: 'Serpentes',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Boidae',
      sous_famille: 'Boinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Titanoboa',
      espèces: 'Titanoboa cerrejonensis',
    },
    regime_alimentaire: 'Piscivore',
    habitat: "Forêts tropicales humides, à proximité d'eau douce",
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Paléogène',
      epoque: 'Paléocène',
      stage: 'Sélandien',
      apparition: -60,
      extinction: -58,
    },
    description:
      "On a pendant longtemps cru que Titanoboa était un superprédateur du fait de sa taille immense pour un serpent et qu'il avait le même mode de chasse qu'un anaconda, à savoir celui d'étouffer ses proies avant de les avaler. Pourtant, avec l'étude des os de son crâne, il s'agissait possiblement plus d'un chasseur de poissons. Ses vertèbres étaient très robustes et larges, lui permettant de se déplacer avec une certaine souplesse.",
    autres_infos: {
      taille: {
        longueur: '12-14 mètres',
        hauteur: '',
      },
      poids: '730-1135 kilogrammes',
    },
    decouverte: {
      date: '2002',
      lieu: 'Colombie',
    },
  },
  {
    nom: 'Arsinoitherium',
    etymologie:
      "Doit son nom à Arsinoé II, une reine lagide d'Égypte dont le palais se situait à proximité du lieu de la découverte de Arsinoitherium zitteli.",
    image_url: './assets/images/Arsinoitherium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: 'Afrotheria',
      ordre: 'Embrithopoda',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Arsinoitheriidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Arsinoitherium',
      espèces: 'Arsinoitherium andrewsi, Arsinoitherium giganteum, Arsinoitherium zitteli',
    },
    regime_alimentaire: 'Végétarien, frugifore',
    habitat: 'Marécages, eaux très peu profondes',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Paléogène',
      epoque: 'Éocène',
      stage: 'Priabonien',
      apparition: -36,
      extinction: -30,
    },
    description:
      "Ce drôle d'animal ressemblant à un rhinocéros, possédait une énorme paire de cornes semblables à des couteaux projetés au-dessus du nez, et une deuxième paire de minuscules cornes, semblables à des boutons sur la tête, placées juste derrière les plus grandes cornes. Son squelette est robuste, et montre qu'il était capable de courir. Arsinoitherium a eu l'assortiment complet de 44 dents. <br /> Sa taille massive le protégeait de la prédation, mais les plus faibles devaient tout de même servir de proie.",
    autres_infos: {
      taille: {
        longueur: '3 mètres',
        hauteur: '1.75-3 mètres',
      },
      poids: '2.5 tonnes',
    },
    decouverte: {
      date: '1902',
      lieu: 'Egypte, Ethiopie',
    },
  },
  {
    nom: 'Bagacératops',
    etymologie: "Visage à petite corne (du mongol baga, 'petit')",
    image_url: './assets/images/Bagaceratops.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Ornithischia',
      sous_ordre: 'Marginocephalia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Ceratopsia',
      famille: 'Bagaceratopidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Bagaceratops',
      espèces: 'Bagaceratops rozhdestvenskyi',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Maastrichtien',
      apparition: -72,
      extinction: -71,
    },
    description:
      'Ce petit dinosaure possédait une toute petite corne sur le museau. Il ne possédait pas de dents de devant.',
    autres_infos: {
      taille: {
        longueur: '1 mètre',
        hauteur: '30 centimètres',
      },
      poids: '22-45 kilogrammes',
    },
    decouverte: {
      date: '1975',
      lieu: 'Mongolie',
    },
  },
  {
    nom: 'Deinocheirus',
    etymologie: 'Terrible main',
    image_url: './assets/images/Deinocheirus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: 'Coelurosauria',
      super_famille: '',
      famille: 'Deinocheiridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Deinocheirus',
      espèces: 'Deinocheirus mirificus',
    },
    regime_alimentaire: 'Omnivore',
    habitat: 'Marécages',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Maastrichtien',
      apparition: -71,
      extinction: -69,
    },
    description:
      "Cet animal est l'un des plus énigmatiques du Crétacé. <br /> Ses bras mesuraient 2.40 mètres et se terminaient pas des griffes de près de 25 centimètres de long. Deinocheirus possédait des pattes postérieures courtes, de grandes épines dorsales et un crâne à « bec de canard » dépourvu de dentition, lié à une alimentation omnivore, principalement composée de plantes herbacées et de petits poissons, ce qui semble être confirmé par les restes retrouvés au niveau de l'estomac.  On y a aussi retrouvé un millier de gastrolithes (cailloux) qui devaient compenser cette absence de dents dans le processus de fragmentation des aliments avalés. <br /> Enfin, les os des pieds sont larges et aplatis, adaptés à la marche en milieu marécageux. <br /> Nous ne savons pas si ces griffes lui servaient à se défendre, manipuler sa nourriture ou même si cela permettait de grimper aux arbres...",
    autres_infos: {
      taille: {
        longueur: '11 mètres',
        hauteur: '4-6 mètres',
      },
      poids: '6 tonnes',
    },
    decouverte: {
      date: '1965',
      lieu: 'Mongolie',
    },
  },
  {
    nom: 'Therizinosaurus',
    etymologie: 'Lézard faucheur',
    image_url: './assets/images/Therizinosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Therizinosauroidea',
      famille: 'Therizinosauridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Therizinosaurus',
      espèces: 'Theriznosaurus cheloniformis',
    },
    regime_alimentaire: 'Omnivore',
    habitat: '',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Maastrichtien',
      apparition: -70,
      extinction: -66,
    },
    description:
      'La plupart des dinosaures théropodes ont des griffes plutôt petites. Leurs bras ne sont pas très puissants. Pourtant, les griffes de Therizinosaurus atteignaient 70 centimètres, ses bras mesurant 2 mètres de long, étaient très solides et particulièrement musclés. <br /> Les os des bras étaient massifs, avec de larges insertions musculaires bien marquées. Ses griffes lui servaient à amener les palntes à sa gueule, mais très certaimement aussi à se défendre. Il pouvait se nourrir de petits lézards ou petits mammifères.',
    autres_infos: {
      taille: {
        longueur: '9 mètres',
        hauteur: '5 mètres',
      },
      poids: '6 tonnes',
    },
    decouverte: {
      date: '1954',
      lieu: 'Mongolie',
    },
  },
  {
    nom: 'Hyaenodon',
    etymologie: 'Dent de hyène',
    image_url: './assets/images/Hyaenodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Eutheria',
      division: '',
      super_ordre: '',
      ordre: 'Creodonta',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Hyaenodontoidea',
      famille: 'Hyaenodontidae',
      sous_famille: 'Hyaenodontinae',
      tribu: 'Hyaenodontini',
      sous_tribu: '',
      genre: 'Hyaenodon',
      espèces:
        'Hyaenodon brachyrhynchus, Hyaenodon chunkhtensis, Hyaenodon dubius, Hyaenodon eminus, Hyaenodon exiguus, Hyaenodon filholi, Hyaenodon gervaisi, Hyaenodon heberti, Hyaenodon leptorhynchus, Hyaenodon minor, Hyaenodon pervagus, Hyaenodon pumilus, Hyaenodon requieni, Hyaenodon rossignoli, Hyaenodon weilini, Hyaenodon yuanchuensis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Paléogène',
      epoque: 'Éocène',
      stage: 'Lutétien',
      apparition: -42,
      extinction: -16.9,
    },
    description:
      "Cet animal ressemblait à une hyène actuelle mais prenait plutôt l'apparence d'un croisement entre un loup et un félin. <br /> Son corps, long et robuste, se terminait par une longue queue. Il possédait cinq orteils à chaque pied et chaque orteil était armé d'une griffe tranchante. Le cou était plus court que le crâne, long et très massif. Celui-ci se terminait par un museau étroit, beaucoup plus grand que ceux des carnivores actuels. <br /> Le crâne de Hyaenodon renfermait un cerveau ridiculement petit par rapport à sa taille, mais sa structure démontre qu'il était doté d'importants muscles maxillaires et d'un sens de l'odorat particulièrement développé. La mâchoire de cet animal féroce était contituée de huit dents pointues, acérées, alignées de chaque côté de la mâchoire et conservaient leur arête tranchante. <br /> Les mâchoires, longues et étroites, étaient puissantes et servaient à mordre, à briser le cou de ses proies, à broyer et à écraser les chaires et les os. Il est possible que Hyaenodon pouvait attendre 55 km/h à la course.",
    autres_infos: {
      taille: {
        longueur: "jusqu'à 3 mètres",
        hauteur: '30-140 centimètres',
      },
      poids: '40-500 kilogrammes',
    },
    decouverte: {
      date: '1838',
      lieu: 'Amérique du nord, Mongolie',
    },
  },
  {
    nom: 'Eustreptospondylus',
    etymologie: 'Colonne vertébrale bien tournée',
    image_url: './assets/images/Eustreptospondylus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Megalosauridae',
      sous_famille: 'Eustreptospondylinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Eustreptospondylus',
      espèces: 'Eustreptospondylus oxoniensis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme, non-loin de la mer',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Supérieur',
      stage: 'Oxfordien',
      apparition: -163,
      extinction: -154,
    },
    description:
      "Sur la base d'un seul fossile d'Eustreptospondylus connu à ce jour, les paléontologues savent que ce dinosaure avait un crâne présentant museau plutôt pointu en vue latérale, avec une grande narine orientée horizontalement. Des rainures obliques dans les articulations des mâchoires permettaient d'élargir l'ouverture de la gueule lorsque les mâchoires inférieures étaient ouvertes. <br /> Même si aucune dent n'a été découverte, nous savons que c'était un animal carnivore. <br /> Certains avancent qu'Eustreptospondylus pouvait nager en eaux peu profondes, pour chasser sur des îles voisines.",
    autres_infos: {
      taille: {
        longueur: '7 mètres',
        hauteur: '3 mètres',
      },
      poids: '200 kilogrammes',
    },
    decouverte: {
      date: '1870',
      lieu: 'Angleterre',
    },
  },
  {
    nom: 'Microraptor',
    etymologie: 'Petit voleur',
    image_url: './assets/images/Microraptor.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Avetheropoda',
      sous_ordre: 'Coelurosauria',
      infra_ordre: 'Deinonychosauria',
      micro_ordre: '',
      super_famille: '',
      famille: 'Dromaeosauridae',
      sous_famille: 'Microraptorinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Microraptor',
      espèces: 'Microraptor gui, Microraptor hanqingi',
    },
    regime_alimentaire: 'Omnivore',
    habitat: 'Forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Aptien',
      apparition: -120,
      extinction: -119,
    },
    description:
      "Microraptor avait un corps recouvert de plumes y compris sur les pattes avant et arrière, avec un éventail de plumes en forme de losange au bout de la longue queue qui contribuait sans doute à assurer une stabilité en vol. Microraptor possédait deux paires d'ailes: En réalité, ses pattes étaient des ailes. Nous ne savons pas exactement si Microraptor pouvait voler ou seulement planer, mais aucune hypothèse n'est à exclure car l'analyse de ses plumes indique que l'animal savait planer mais aurait été capable de voler.",
    autres_infos: {
      taille: {
        longueur: '80 centimètres',
        hauteur: '20 centimètres',
      },
      poids: '1-2 kilogrammes',
    },
    decouverte: {
      date: '2001',
      lieu: 'Chine',
    },
  },
  {
    nom: 'Chalicotherium',
    etymologie: 'Animal de gravier',
    image_url: './assets/images/Chalicotherium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Eutheria',
      division: '',
      super_ordre: '',
      ordre: 'Perissodactyla',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Chalicotheriidae',
      sous_famille: 'Chalicotheriinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Chalicotherium',
      espèces: 'Chalicotherium brevirostris, Chalicotherium goldfussi, Chalicotherium salinum',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme, forêts',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Langhien',
      apparition: -15,
      extinction: -8,
    },
    description:
      "Chalicotherium était un animal unique, combinant des caractéristiques de plusieurs espèces: Il se déplaçait à la manière d'un gorille en utilisant ses articulations, ses bras lui permettant d'atteindre les branches des grands arbres pour se nourrir, à la manière d'un panda. <br /> Ses griffes étaient exceptionnellement longues, qu'il utilisait pour dénuder les feuilles des branches. Les callosités sur son ischion indiquent qu'il passait de longues périodes assis sur ses hanches, probablement en train de se nourrir. Pour éviter l'usure de ses griffes, il marchait sur ses articulations, ce qui lui permettait de les préserver pour une utilisation ultérieure, soit pour collecter de la nourriture, soit comme des armes défensives redoutables, ou même les deux en même temps.",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '2.50 mètres',
      },
      poids: '1.5 tonne',
    },
    decouverte: {
      date: '1833',
      lieu: 'Allemagne',
    },
  },
  {
    nom: 'Mégatherium',
    etymologie: 'Animal géant',
    image_url: './assets/images/Megatherium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: 'Xenarthra',
      ordre: 'Pilosa',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Megatheriidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Megatherium',
      espèces:
        'Megatherium altiplanicum, Megatherium americanum, Megatherium gallardoi, Megatherium istilarti, Megatherium lundi-sejoi, Megatherium medinae, Megatherium parodii, Megatherium sundti, Megatherium tarijense',
    },
    regime_alimentaire: 'Omnivore',
    habitat: 'Terre ferme, régions peu boisées et semi-arides',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Pliocène',
      stage: 'Zancléen',
      apparition: -5,
      extinction: -0.01,
    },
    description:
      "Mégatherium n'est ni plus ni moins qu'un paresseux géant. <br /> Principalement quadrupède, il pouvait de dresser sur deux pattes et sa queue lui servait peut-être d'appui quand il se dressait en position verticale, pour mieux attraper les feuilles hautes. S'élevant sur ses puissantes pattes postérieures et utilisant donc sa queue, Mégatherium pouvait supporter le poids massif de son corps tout en utilisant les griffes incurvées de ses longues pattes antérieures pour arracher les branches contenant les feuilles les plus recherchées. La mâchoire ce cet animal lui permettait également de mâcher les fruits. Il est aussi possible qu'il lui arrivait de manger de la viande, certainement des charognes fraîches. <br /> Mégatherium marchait sur les côtés de ses pieds car ses griffes l'empêchaient de les poser à plat sur le sol. <br /> Une étude a proposé que Mégatherium était principalement dépourvu de poils, parce que sa grande taille et son faible rapport surface/volume l'auraient rendu vulnérable à la surchauffe de son corps, mais cela reste à prouver.",
    autres_infos: {
      taille: {
        longueur: '6 mètres',
        hauteur: '',
      },
      poids: '4 tonnes',
    },
    decouverte: {
      date: '1788',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Aurochs',
    etymologie: 'Bœuf sauvage',
    image_url: './assets/images/Aurochs.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Artiodactyla',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Bovidae',
      sous_famille: 'Bovinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Bos',
      espèces:
        'Bos primigenius primigenius, Bos primigenius namadicus, Bos primigenius mauritanicus, Bos primigenius thrinacius',
    },
    regime_alimentaire: 'Herbivore',
    habitat: 'Plaines',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Quaternaire',
      epoque: 'Pléistocène',
      stage: 'Gélasien',
      apparition: -2,
      extinction: 1720,
    },
    description: "Il s'agit d'une vache XXL. <br /> Auroch menait le même mode de vie que nos vaches actuelles.",
    autres_infos: {
      taille: {
        longueur: '2.5-3 mètres',
        hauteur: '1.60-1.80 mètre',
      },
      poids: '800-1000 kilogrammes',
    },
    decouverte: {
      date: '',
      lieu: 'Europe, Inde',
    },
  },
  {
    nom: 'Mégaloceros',
    etymologie: 'Brame puissant',
    image_url: './assets/images/Megaloceros.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Artiodactyla',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Cervidae',
      sous_famille: 'Cervinae',
      tribu: 'Cervini',
      sous_tribu: '',
      genre: 'Megaloceros',
      espèces:
        'Megaloceros antecedens, Megaloceros giganteus, Megaloceros luochuanensis, Megaloceros matritensis, Megaloceros novocarthaginiensis, Megaloceros savini',
    },
    regime_alimentaire: 'Herbivore',
    habitat: 'Forêts',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Quaternaire',
      epoque: 'Pléistocène',
      stage: 'Calabrien',
      apparition: -1.2,
      extinction: -0.077,
    },
    description:
      "Anciennement appelé 'Mégaceros', Mégalocéros arborait des bois gigantesques dont l'envegure pouvait atteindre 3.5 mètres et peser une quarantaine de kilogrmames. Les bois des femelles étaient 10 % plus petits.",
    autres_infos: {
      taille: {
        longueur: '2.5 mètres',
        hauteur: '2 mètres',
      },
      poids: '700 kilogrammes',
    },
    decouverte: {
      date: '1828',
      lieu: 'Eurasie',
    },
  },
  {
    nom: 'Odobénocétops',
    etymologie: 'Baleine morse',
    image_url: './assets/images/Odobenocetops.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      infra_classe: 'Placentalia',
      division: '',
      super_ordre: '',
      ordre: 'Cetartiodactyla',
      sous_ordre: '',
      infra_ordre: 'Cetacea',
      micro_ordre: 'Odontoceti',
      super_famille: 'Delphinoidea',
      famille: 'Odobenocetopsidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Odobenocetops',
      espèces: 'Odobenocetops leptodon, Odobenocetops peruvianus',
    },
    regime_alimentaire: 'Fouisseur',
    habitat: 'Mers peu profondes',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Aquitanien',
      apparition: -23,
      extinction: -10,
    },
    description:
      "Cette baleine préhistorique est connu pour ses défenses atypiques orientées vers l'arrières. <br /> Chez le mâle, ces défenses sont asymétriques. La défense droite est rectiligne et mesure jusqu'à 1,35 mètre de long ; lors de la nage elle devait rester en position parallèle au corps de l'animal. La défense gauche ne mesure qu'une trentaine de centimètres. <br /> La femelle ne possède que deux petites défenses de taille égale. Son museau est particulièrement court et nettement dirigé vers le bas tandis que ses narines sont au contraire rejetées en arrière vers le haut du museau, de plus son condyle occipital est extrêmement saillant ce qui est révélateur d'une grande mobilité du cou. Ces trois éléments anatomiques témoignent d'un animal qui se nourrissait sur le fond de la mer. <br /> Odobénocétops devait se nourrir de coquillages enfouis dans les sédiments de la mer. Il utilisait probablement sa langue et ses lèvres supérieures puissantes pour aspirer par succion les parties molles de ses proies.",
    autres_infos: {
      taille: {
        longueur: '3-4 mètres',
        hauteur: '',
      },
      poids: '800-1000 kilogrammes',
    },
    decouverte: {
      date: '1990',
      lieu: 'Pérou',
    },
  },
  {
    nom: 'Smilodon',
    etymologie: 'Dent en ciseaux',
    image_url: './assets/images/Smilodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Carnivora',
      sous_ordre: 'Feliformia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Felidae',
      sous_famille: 'Machairodontinae',
      tribu: 'Smilodontini',
      sous_tribu: '',
      genre: 'Smilodon',
      espèces: 'Smilodon fatalis, Smilodon gracilis, Smilodon populator',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Plaines et forêts',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Quaternaire',
      epoque: 'Pléistocène',
      stage: 'Gélasien',
      apparition: -2.5,
      extinction: -0.01,
    },
    description:
      "Très souvent appelé 'Tigre aux dents de sable', Smilodon tient son nom de ses longues canines supérieurs pouvant mesurer jusqu'à 30 centimètres. Il chassait comme les lions actuels, en plantant des canines dans la gorge de ses proies pour les saigner rapidement. <br /> Il s'agissait du prédateur le plus répendu et le plus redouté de son époque; il régnait en maître sur tout le continent américain. ",
    autres_infos: {
      taille: {
        longueur: '1.80-2.30 mètres',
        hauteur: '1.20 mètre',
      },
      poids: '55-470 kilogrammes',
    },
    decouverte: {
      date: '1830',
      lieu: 'Amérique',
    },
  },
  {
    nom: 'Ours des cavernes',
    etymologie: '--',
    image_url: './assets/images/UrsusSpelaeus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      cohorte: 'Placentalia',
      division: '',
      super_ordre: '',
      ordre: 'Carnivora',
      sous_ordre: 'Caniformia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Ursidae',
      sous_famille: 'Ursinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Ursus',
      espèces: 'Ursus spelaeus',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Forêts mixtes',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Quaternaire',
      epoque: 'Holocène',
      stage: 'Northgrippien',
      apparition: -0.043,
      extinction: -0.024,
    },
    description:
      "L'Ours des cavernes est un animal robuste, puissant et féroce. <br /> Plus à l'aise au déplacement à quatre pattes, il n'hésitait pas à se dresser sur ses pattes arrières pour impressionner quiconque s'attaquait à lui. Les pattes avant sont plus longues et plus robustes que les pattes arrière, ce qui donne à l'animal un profil surbaissé à l'arrière-train. Son crâne était très large avec un front prononcé. <br /> Quant à sa mâchoire, il possédait des molaires allongées valant pour 3 molaires des ours actuels. L'Ours des cavernes avait le même mode de vie que les ours bruns actuels, à savoir des periodes d'activité et des périodes d'hibernation. ",
    autres_infos: {
      taille: {
        longueur: '3.50 mètres',
        hauteur: '1.30 mètre',
      },
      poids: '470-900 kilogrammes',
    },
    decouverte: {
      date: '1774',
      lieu: 'Europe',
    },
  },
  {
    nom: 'Leedsichthys',
    etymologie: 'Poisson de Leeds (du nom de son découvreur)',
    image_url: './assets/images/Leedsichthys.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Actinopterygii',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Pachycormiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Pachycormidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Leedsichthys',
      espèces: 'Leedsichthys problematicus',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Moyen',
      stage: 'Callovien',
      apparition: -165,
      extinction: -152,
    },
    description:
      "Malgré sa taille gigantesque, Leedsichthys était un poisson inoffensif. <br /> Sa grande taille, qui fait de lui l'un des plus gros poissons ayant existé, le protégeait des prédateurs. Il devait avoir un métabolisme permettant une excellente cicatrisation. <br /> Sa nageaoire caudale dégageait suffisament de puissance pour assomer les prédateurs qui s'approchaient trop près de lui.",
    autres_infos: {
      taille: {
        longueur: '9-27 mètres',
        hauteur: '2-3 mètres',
      },
      poids: '30-50 tonnes',
    },
    decouverte: {
      date: '1886',
      lieu: 'Angleterre',
    },
  },
  {
    nom: 'Elasmotherium',
    etymologie: "Lame d'animal sauvage",
    image_url: './assets/images/Elasmotherium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Perissodactyla',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Rhinocerotidae',
      sous_famille: 'Elasmotheriinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Elasmotherium',
      espèces:
        'Elasmotherium caucasicum, Elasmotherium chaprovicum, Elasmotherium peii, Elasmotherium primigenium, Elasmotherium sibiricum',
    },
    regime_alimentaire: 'Herbivore',
    habitat: 'Steppes, plaines semi-arides',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Messinien',
      apparition: -7,
      extinction: -0.039,
    },
    description:
      "Elasmotherium avait des pattes plus longues que celles des rhinocéros actuels. Elle étaient faites pour le galop, ce qui donnait à l'animal une allure de cheval. D'ailleurs, il est raisonnable de penser que cet animal pouvait courir à plus de 55 km/h, soit plus que nos rhinocéros actuels. <br /> On représente bien souvent Elasmotherium avec une corne géante, de près de 2 mètres. Cependant, en 2021, une équipe de paléotologues a réétudié la morphologie de son crâne: ils ont exclu la présence de cette corne gigantesque en raison du dôme osseux relativement fragile qui ne pouvait probablement pas supporter le poids d’une corne de 2 mètres. Ils envisagent également la présence d’une petite zone kératinisée en position nasale, ce qui conférerait à Elasmotherium non pas une mais deux « pseudo-cornes » sur le crâne. <br /> Ses dents indiquent probablement un régime à base d'herbes coriaces. <br /> C'est animal présentait également une fourrure à poils longs.",
    autres_infos: {
      taille: {
        longueur: '5 mètres',
        hauteur: '2 mètres',
      },
      poids: '4 tonnes',
    },
    decouverte: {
      date: '1807',
      lieu: 'Russie',
    },
  },
  {
    nom: 'Diprotodon',
    etymologie: "Deux dents vers l'avant",
    image_url: './assets/images/Diprotodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Metatheria',
      division: '',
      super_ordre: '',
      ordre: 'Diprotodontia',
      sous_ordre: 'Vombatiformes',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Diprotodontidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Diprotodon',
      espèces: 'Diprotodon optatum',
    },
    regime_alimentaire: 'Herbivore',
    habitat: 'Forêts, savanes',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Quaternaire',
      epoque: 'Pléistocène',
      stage: 'Calabrien',
      apparition: -1.78,
      extinction: -0.04,
    },
    description:
      "Diprotodon est le plus grand marsupial connu. <br /> Très peu agile, il ne devait pas se déplacer très vite. Du fait de sa taille massive, il n'avait que très peu de prédation. <br /> Il possédait deux dents incisives de 30 centimètres à l'avant de la mâchoire ce qui lui permettait de manger des herbes plus coriaces. <br /> Il y a de fortes probabilités que Diprotodon ait été un animal migrateur.",
    autres_infos: {
      taille: {
        longueur: '3 mètres',
        hauteur: '2 mètres',
      },
      poids: ' 3 tonnes',
    },
    decouverte: {
      date: '1830',
      lieu: 'Autralie',
    },
  },
  {
    nom: 'Glyptotherium',
    etymologie: 'Animal sculpté',
    image_url: './assets/images/Glyptotherium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Eutheria',
      division: '',
      super_ordre: 'Xenarthra',
      ordre: 'Cingulata',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Glyptodontidae',
      sous_famille: 'Glyptodontinae',
      tribu: 'Glyptotheriini',
      sous_tribu: '',
      genre: 'Glyptotherium',
      espèces: '',
    },
    regime_alimentaire: 'Herbivore',
    habitat: 'Plaines semi-arides',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Pliocène',
      stage: 'Zancléen',
      apparition: -4.9,
      extinction: -0.007,
    },
    description:
      "Cet animal est d'apparence un mélange entre un tatou et une tortue. Sa carapace épaisse est un excellemnt bouclier contre les prédateurs. Le sommet de sa tête était aussi doté d'un petit bouclier. <br /> La carapace du Glyptotherium était constituée de centaines de petites écailles hexagonales. <br /> Son squelette axial présente une fusion étendue de la colonne vertébrale et le bassin est fusionné à la carapace, ce qui rend le bassin entièrement immobile <br />. Sa grande queue servait probablement de contrepoids au reste du corps et cette armure dorsale se terminait par un tube émoussé composé de 2 ou 3 tubes fusionnés.",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '',
      },
      poids: '350-1165 kilogrammes',
    },
    decouverte: {
      date: '1870',
      lieu: 'Mexique, Argentine',
    },
  },
  {
    nom: 'Doedicurus',
    etymologie: 'Queue en forme de pilon',
    image_url: './assets/images/Doedicurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Placentalia',
      division: '',
      super_ordre: 'Xenarthra',
      ordre: 'Cingulata',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Chlamyphoridae',
      sous_famille: 'Glyptodontinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Doedicurus',
      espèces: 'Doedicurus clavicaudatus',
    },
    regime_alimentaire: 'Herbivore',
    habitat: 'Prairies',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Quaternaire',
      epoque: 'Pléistocène',
      stage: 'Gélasien',
      apparition: -2,
      extinction: -0.007,
    },
    description:
      "Doedicurus était un animal rond, doté d'une lourde armure et d'une carapace. Il portait notamment une massue à pointes sur la queue, qui pouvait peser 40 ou 65 kg, qu'il pouvait brandir pour se défendre contre les prédateurs ou lors de combats avec d'autres Doedicurus à une vitesse d'environ 11 m/s (40 km/h). <br /> Le champ de vision de l’animal étant très limité, il aurait été contraint à balancer sa queue au hasard. <br /> La carapace était solidement attachée au bassin, mais moins étroitement aux épaules. Cet emplacement portait un bouclier additionnel plus petit et les paléontologues en ont déduit l’existence d’une bosse graisseuse, semblable à la bosse d'un dromadaire, qui aurait servi à l’animal à stocker de l’énergie en vue de la saison sèche tout en amortissant les chocs lors d’affrontements avec un rival.",
    autres_infos: {
      taille: {
        longueur: '4 mètres',
        hauteur: '1.50 mètres',
      },
      poids: '3 tonnes',
    },
    decouverte: {
      date: '1947',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Tapejara',
    etymologie: "'Vieil être', en langue tupi",
    image_url: './assets/images/Tapejara.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Pterosauria',
      sous_ordre: 'Pterodactyloidea',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Tapejaridae',
      sous_famille: 'Tapejarinae',
      tribu: 'Tapejarini',
      sous_tribu: '',
      genre: 'Tapejara',
      espèces: 'Tapejara wellnhoferi',
    },
    regime_alimentaire: 'Omnivore',
    habitat: 'Falaises',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Albien',
      apparition: -112,
      extinction: -110,
    },
    description:
      "Ce petit ptérosaure est caractérisé par sa crête ronde en forme d’œuf penché qui mesurait 30 centimètres de haut. <br /> Pour voler, il portait une membrane de peau renfermant une fine couche de muscle aussi fine qu'une feuille de papier et ses os étaient creux, ce qui explique son poids léger. <br /> Son bec puissant lui permettait de dévorer des graines, des poissons, des insectes et des coquillages.",
    autres_infos: {
      taille: {
        longueur: '80 centimètres',
        hauteur: '50 centimètres',
        envergure: '3 mètres',
      },
      poids: '5 kilogrammes',
    },
    decouverte: {
      date: '1989',
      lieu: 'Brésil',
    },
  },
  {
    nom: 'Ornithocheirus',
    etymologie: "Main d'oiseau",
    image_url: './assets/images/Ornithocheirus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Pterosauria',
      sous_ordre: 'Pterodactyloidea',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Ornithocheiridae',
      sous_famille: 'Ornithocheirinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Ornithocheirus',
      espèces: 'Ornithocheirus simus',
    },
    regime_alimentaire: 'Piscivore, charognard',
    habitat: 'Falaises',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Albien',
      apparition: -105,
      extinction: -100,
    },
    description:
      "Ornithocheirus n'est connu que par des fragments de mâchoire. <br /> Au bout de son bec se trouvait une crête de forme bi-convexe, comme des demi-lunes. On a longtemps pensé que ce ptérosaure était le plus grand n'ayant jamais existé. En réalité, il entre la catégorie des ptérosaures de taille moyenne, son envergure ayant été exagérément calculée entre 12 et 14 mètres.",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '',
        envergure: '4-5 mètres',
      },
      poids: '20 kilogrammes',
    },
    decouverte: {
      date: '19è siècle',
      lieu: 'Angleterre',
    },
  },

  {
    nom: 'Herrerasaurus',
    etymologie: 'Lézard de Herrera',
    image_url: './assets/images/Herrerasaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Herrerasauridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Herrerasaurus',
      espèces: 'Herrerasaurus ischigualastensis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Trias',
      epoque: 'Supérieur',
      stage: 'Carnien',
      apparition: -231.4,
      extinction: -228.9,
    },
    description:
      "Herrerasaurus était l'un des tous premiers dinosaures. <br /> Il avait un crâne unique, semblable à celui des archosauriformes primitifs, avec cinq paires de fenêtres, y compris des fenêtres antéorbitaires et des fenêtres promaxillaires. Sa mâchoire inférieure avait une articulation souple pour saisir les proies. <br /> Les mâchoires étaient munies de dents en forme de scie pour déchiqueter la chair. <br /> Son cou était mince et flexible. <br /> Les pattes avant d'Herrerasaurus étaient courtes, avec des doigts munis de griffes pour saisir, tandis que les pattes postérieures étaient fortes et adaptées à la course. Il avait cinq orteils, mais seuls les trois du milieu portaient son poids. Les orteils extérieurs étaient petits, avec un petit griffon sur le premier orteil. <br /> Sa queue, partiellement raide, servait à l'équilibre et à la vitesse. ",
    autres_infos: {
      taille: {
        longueur: '3-5 mètres',
        hauteur: '1 mètre',
      },
      poids: '210-350 kilogrammes',
    },
    decouverte: {
      date: '1961',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Longisquama',
    etymologie: 'Logues écailles',
    image_url: './assets/images/Longisquama.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: 'Diapsida',
      division: '',
      super_ordre: '',
      ordre: 'Prolacertiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Longisquamidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Longisquama',
      espèces: 'Longisquama insignis',
    },
    regime_alimentaire: 'Insectivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Trias',
      epoque: 'Supérieur',
      stage: 'Carnien',
      apparition: -235,
      extinction: -220,
    },
    description:
      "Longisquama est un petit reptile bien reconnaissable à ses écailles extrêmement allongées qui forment une sorte de voile dorsale. <br /> Il devait pouvoir la redresser pour effrayer ses prédateurs ou attirer ses partenaires. Mais certains paléontologues positionnent autrement ces longues écailles, et supposent qu'elles lui servaient plutôt à voler. <br /> Il avait de longs doigts crochus surmontés de petites griffes.",
    autres_infos: {
      taille: {
        longueur: '15-20 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1970',
      lieu: 'Kirghizistan',
    },
  },
  {
    nom: 'Tanystropheus',
    etymologie: 'Longues vertèbres',
    image_url: './assets/images/Tanystropheus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Tanystropheidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Tanystropheus',
      espèces: 'Tanystropheus conspicuus, Tanystropheus hydroides, Tanystropheus longobardicus',
    },
    regime_alimentaire: 'Piscivore',
    habitat: 'Eaux marines peu profondes',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Trias',
      epoque: 'Moyen',
      stage: 'Ladinien',
      apparition: -240,
      extinction: -220,
    },
    description:
      "Le cou de cet animal est sans conteste sa particularité anatomique la plus surprenante : il pouvait atteindre jusqu’à 3 mètres de long, soit la moitié de la longueur de son corps, sachant qu'il possédait aussi une longue queue ! Ce cou possède 13 vertèbres. Une étude, conduite en 2015 par le paléontologue britannique Mark Witton, renforce l'hypothèse d'une vie à dominante terrestre: Tanystropheus ne présente pas d'adaptation significative à la vie aquatique. <br /> De plus, il estime que le poids du cou, malgré sa longueur démesurée, ne représente que 20 % de la masse totale de l'animal et parait donc facilement manœuvrable. Il conclut que Tanystropheus aurait pêché à la manière d'un héron.",
    autres_infos: {
      taille: {
        longueur: '6 mètres',
        hauteur: '',
      },
      poids: '30-80 kilogrammes',
    },
    decouverte: {
      date: '1852',
      lieu: 'Italie',
    },
  },
  {
    nom: 'Epidexipteryx',
    etymologie: "Plume d'affichage",
    image_url: './assets/images/Epidexipteryx.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: 'Coelurosauria',
      super_famille: '',
      famille: 'Scansoriopterygidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Epidexipteryx',
      espèces: 'Epidexipteryx hui',
    },
    regime_alimentaire: 'Omnivore',
    habitat: 'Forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Moyen',
      stage: 'Callovien',
      apparition: -164,
      extinction: -163,
    },
    description:
      "Ce petit dinosaure possédait un crâne où les dents sont uniquement implantées à l'avant de la mâchoire, et celles de devant sont longues et inclinées vers l'avant. <br /> La queue d’Epidexipteryx possède des vertèbres inhabituelles vers la pointe qui ressemble au pygostyle (un os du croupion) des oiseaux modernes et des oviraptorosaures.  <br /> Quant aux plumes, eslles sont uniques par le fait qu'elles semblent surgir d'une « structure membraneuse » située à leur base.",
    autres_infos: {
      taille: {
        longueur: '30 centimètres',
        hauteur: '15 centimètres',
      },
      poids: '170 grammes',
    },
    decouverte: {
      date: '2008',
      lieu: 'Chine',
    },
  },
  {
    nom: 'Cryolophosaurus',
    etymologie: 'Lézard à crête de glace',
    image_url: './assets/images/Cryolophosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Sinraptoridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Cryolophosaurus',
      espèces: 'Cryolophosaurus ellioti',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Moyen',
      stage: 'Pliensbachien',
      apparition: -189,
      extinction: -183,
    },
    description:
      "Cryolophosaurus se caractérise par la présence d'une crête sur le crâne, recourbée vers l'arrière. <br /> Découvert dans l'actuel Pôle Sud, cela ne signifie pas qu'il vivait dans la neige. A cette époque, l'Antarctique se situait au niveau de l'Équateur, donc le climat était plus clément. <br /> Il s'agissait du plus grand prédateur de son temps.",
    autres_infos: {
      taille: {
        longueur: '7 mètres',
        hauteur: '3 mètres',
      },
      poids: '350–465 kilogrammes',
    },
    decouverte: {
      date: '1991',
      lieu: 'Antarctique',
    },
  },
  {
    nom: 'Tupandactylus',
    etymologie: 'Doigt de Tupan, en référence au dieu tupi du tonnerre',
    image_url: './assets/images/Tupandactylus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: 'Diapsida',
      infra_classe: 'Archosauromorpha',
      division: '',
      super_ordre: '',
      ordre: 'Pterosauria',
      sous_ordre: 'Pterodactyloidea',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Azhdarchoidea',
      famille: 'Tapejaridae',
      sous_famille: 'Tapejarinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Tupandactylus',
      espèces: 'Tupandactylus imperator, Tupandactylus navigans',
    },
    regime_alimentaire: 'Piscivore',
    habitat: 'Falaises',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Aptien',
      apparition: -112,
      extinction: -113,
    },
    description:
      "Tupandactylus se distingue par sa grande crête crânienne, composée en partie d'os et en partie de tissus mous. Les crêtes des Tupandactylus sont constituées d'une crête semi-circulaire au-dessus du bec. Cette crête devait probablement servir de signe de reconnaissance entre individus.",
    autres_infos: {
      taille: {
        longueur: '',
        hauteur: '',
        envergure: '3-4 mètres',
      },
      poids: '',
    },
    decouverte: {
      date: '2006',
      lieu: '',
    },
  },
  {
    nom: 'Macrauchenia',
    etymologie: "Grand lama, basé sur le genre de lama désormais invalide, Auchenia, du grec 'grand cou' ",
    image_url: './assets/images/Macrauchenia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Litopterna',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Macraucheniidae',
      sous_famille: 'Macraucheniinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Macrauchenia',
      espèces:
        'Macrauchenia boliviensis, Macrauchenia formosa, Macrauchenia intermedia, Macrauchenia patachonica, Macrauchenia ullomensis',
    },
    regime_alimentaire: 'Herbivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Messinien',
      apparition: -7,
      extinction: -0.01,
    },
    description:
      "Les fossiles de Macrauchenia montrent des narines situées sur le haut du crâne, entre les yeux. Il est alors possible qu’il possédait une trompe semblable à celle d'un tapir ou un museau gonflé comme celui de l'antilope saïga, peut-être pour empêcher la poussière d'entrer dans les narines. La trompe leur servait comme une lèvre préhensile pour attraper, déplacer des objets et comme outil d'usage général. <br /> Selon les études réalisées sur la structure des articulations des pattes, Macrauchenia était habile pour esquiver ses ennemis, et devait courrir vite.",
    autres_infos: {
      taille: {
        longueur: '3 mètres',
        hauteur: '1.80 mètre',
      },
      poids: '1 tonne',
    },
    decouverte: {
      date: '1838',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Andrewsarchus',
    etymologie: 'Maître de Andrews',
    image_url: './assets/images/Andrewsarchus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Eutheria',
      division: '',
      super_ordre: '',
      ordre: 'Artiodactyla',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Andrewsarchidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Andrewsarchus',
      espèces: 'Andrewsarchus mongoliensis',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Paléogène',
      epoque: 'Éocène',
      stage: 'Bartonien',
      apparition: -41,
      extinction: -35,
    },
    description:
      "Seul son crâne de 90 centimètres de long peut nous faire immaginer l'impitoyable carnassier qu'était Andrewsarchus. <br /> Il devait s'agit d'un des plus grand carnivore terrestre de tous les temps. Il y a fort à parier que les pieds d'Andrewsarchus n'étaient pas terminés par des griffes mais par des sabots.",
    autres_infos: {
      taille: {
        longueur: '4 mètres',
        hauteur: '2 mètres',
      },
      poids: '800-1000 kilogrammes',
    },
    decouverte: {
      date: '1923',
      lieu: 'Chine',
    },
  },
  {
    nom: 'Megistotherium',
    etymologie: 'La plus grande bête',
    image_url: './assets/images/Megistotherium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Hyaenodonta',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Hyainailouridae',
      sous_famille: 'Hyainailourinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Megistotherium',
      espèces: 'Megistotherium osteothlastes',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Burdigalien',
      apparition: -19,
      extinction: -14,
    },
    description:
      "Le crâne de cet animal, qui était le plus grand mammifère carnivore de son temps, mesurait entre 65 et 70 centimètres. <br /> Les dents de carnassier du Megisotherium étaient des molaires supérieures, formant des ciseaux avec les molaires inférieures qui coïncidaient entres elles pour former une formidable et puissante action de cisaillement. On a retrouvé des os de mastodonte qui portaient des traces de morsures causées par Megistotherium, ce qui montre que cet animal n'avait peur de rien. <br /> Il y a fort à parier que les pieds de Megistotherium n'étaient pas terminés par des griffes mais par des sabots.",
    autres_infos: {
      taille: {
        longueur: '3-4 mètres',
        hauteur: '1.80 mètre',
      },
      poids: '500-850 kilogrammes',
    },
    decouverte: {
      date: '1973',
      lieu: 'Égypte, Libye',
    },
  },
  {
    nom: 'Procoptodon',
    etymologie: '--',
    image_url: './assets/images/Procoptodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      infra_classe: 'Marsupialia',
      division: '',
      super_ordre: '',
      ordre: 'Diprotodontia',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Macropodidae',
      sous_famille: 'Sthenurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Procoptodon',
      espèces:
        'Procoptodon cegsai, Procoptodon gilli, Procoptodon goliah, Procoptodon maddocki, Procoptodon mccoy, Procoptodon pusio, Procoptodon texasensis, Procoptodon williamsi',
    },
    regime_alimentaire: 'Herbivore',
    habitat: 'Terre ferme, forêts',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Quaternaire',
      epoque: 'Pléistocène',
      stage: 'Gélasien',
      apparition: -2,
      extinction: -0.045,
    },
    description:
      "Ce kangourou géant avait une face courte et des yeux orientés vers l'avant. <br /> Chaque pied avait un seul doigt, à la façon du cheval actuel. Ils pouvaient ainsi se déplacer rapidement dans les forêts et les prairies où ils trouvaient herbes et feuilles pour se nourrir. Leurs pattes avant étaient aussi originales : elles portaient deux très longs doigts qui se terminaient par de longues griffes. On pense qu'elles servaient à attraper les branches situées à grande hauteur pour pouvoir les porter à la bouche.",
    autres_infos: {
      taille: {
        longueur: '',
        hauteur: '1-2.70 mètres',
      },
      poids: '240-500 kilogrammes',
    },
    decouverte: {
      date: '1873',
      lieu: 'Australie',
    },
  },
  {
    nom: 'Giganotosaurus',
    etymologie: 'Lézard géant du Sud',
    image_url: './assets/images/Giganotosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: 'Dinosauria',
      division: '',
      super_ordre: '',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: 'Carnosauria',
      micro_ordre: '',
      super_famille: 'Allosauroidea',
      famille: 'Carcharodontosauridae',
      sous_famille: 'Carcharodontosaurinae',
      tribu: 'Giganotosaurini',
      sous_tribu: '',
      genre: 'Giganotosaurus',
      espèces: 'Giganotosaurus carolinii',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Cénomanien',
      apparition: -99.6,
      extinction: -95,
    },
    description:
      "Giganotosaurus est le plus grand carnivore que notre planète ait porté ! Il a supplanté le célébre Tyranosaurus Rex sous tous les rapports anatomiques. <br /> Giganotosaurus chassait seul ou en groupe, mais dans tous les cas, ce dinosaure n'avait pas peur de s'en prendre à des proies nettement plus massives que lui. <br /> Ses dents acérées mesuraient jusqu'à 20 centimètres. <br /> Sa vision binoculaire était plutôt réduite mais sa vue était plutôt développée. Il se pourrait donc que ce prédateur ait eu une vision plutôt adaptée à la chasse en embuscade et intensive. <br /> Sa gueule et ses pattes puissantes lui permettaient d'infliger des blessures mortelles à ses proies.",
    autres_infos: {
      taille: {
        longueur: '13-15 mètres',
        hauteur: '3.50-4.50 mètres',
      },
      poids: '8-10 tonnes',
    },
    decouverte: {
      date: '1993',
      lieu: 'Patagonie, Argentine',
    },
  },
  {
    nom: 'Argentinosaurus',
    etymologie: 'Lézard argentin',
    image_url: './assets/images/Argentinosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: 'Dinosauria',
      division: '',
      super_ordre: '',
      ordre: 'Saurischia',
      sous_ordre: 'Sauropodomorpha',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Sauropoda',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Argentinosaurus',
      espèces: 'Argentinosaurus huinculensis',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Cénomanien',
      apparition: -96.2,
      extinction: -92.19,
    },
    description:
      "Argentinosaurus est à ce jour le plus massif de tous les dinosaures. Extrêmement lourd il ne déplaçait qu'à 7km/h, soit la vitesse d'un humain en marche rapide. <br /> La plus grande de ces vertèbres mesuraient 1.60 mètre de haut et 1.30 mètre de large, ce qui nous donne une idée de sa colonne vertébrale gigantesque. <br /> Il devait se délacer en troupeau d'une dizaines d'individus pour accroitre les chances de survie de chacun. Il est fort probable que dès qu'un troupeau d'Argentinosaurus approchait, la terre devait trembler.",
    autres_infos: {
      taille: {
        longueur: '30-40 mètres',
        hauteur: '8 mètres',
      },
      poids: '90-100 tonnes',
    },
    decouverte: {
      date: '1987',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Dilophosaurus',
    etymologie: 'Lézard à deux crêtes',
    image_url: './assets/images/Dilophosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: 'Dinosauria',
      division: '',
      super_ordre: '',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Dilophosaurus',
      espèces: 'Dilophosaurus breedorum, Dilophosaurus wetherilli',
    },
    regime_alimentaire: 'Carnivore, charognard',
    habitat: '',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Inférieur',
      stage: 'Pliensbachien',
      apparition: -183.7,
      extinction: -181,
    },
    description:
      " C'est l'un des plus anciens grands dinosaures prédateurs. La caractéristique principale de Dilophosaurus était son crâne qui possédait des articulations lui permettant de plisser le nez et une paire de crêtes osseuses sur la tête (d’où son nom). <br /> Les crêtes de Dilophosaurus, pouvaient peut-être servir d’accessoires visuels de parade. Dilophosaurus présentait un dysmorphisme sexuel avec cette crête, plus grande chez les mâles.<br /> Sa gueule était particulièrement étroite et souple, ses dents fragiles laissent penser que c'était un animal charognard. Mais il est très vraisemblable qu'il chassait et tuait sa victime à l'aide des griffes qu'il portait aux pattes avant et arrière. Sa morphologie permettait au dilophosaure de chasser autant des proies imposantes comme Plateosaurus que des proies beaucoup plus petites comme des petits mammifères et des lézards qu’il allait chercher dans les broussailles ou les crevasses des rochers. <br /> Souvent représenté dans de célèbres films, Dilophosaurus ne possédait pas de colerette.",
    autres_infos: {
      taille: {
        longueur: '6 mètres',
        hauteur: '2 mètres',
      },
      poids: '500 kilogrammes',
    },
    decouverte: {
      date: '1942',
      lieu: 'Arizona',
    },
  },
  {
    nom: 'Mégaraptor',
    etymologie: 'Voleur géant',
    image_url: './assets/images/Megaraptor.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Megaraptor',
      espèces: 'Megaraptor namunhuaiquii',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme, forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Cénomanien',
      apparition: -90,
      extinction: -88,
    },
    description:
      'Encore très peu connu des paléontologues, ce dinosaure se distingue par sa griffe de 30 centimètres de long au premier doigt de chaque main.',
    autres_infos: {
      taille: {
        longueur: '8 mètres',
        hauteur: '3 mètres',
      },
      poids: '1 tonne',
    },
    decouverte: {
      date: '1998',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Protognathosaurus',
    etymologie: 'Lésard à première mâchoire',
    image_url: './assets/images/Protognathosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Sauropodomorpha',
      infra_ordre: 'Sauropoda',
      micro_ordre: '',
      super_famille: '',
      famille: 'Euhelopodidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Protognathosaurus',
      espèces: 'Protognathosaurus oxyodon',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Moyen',
      stage: 'Bathonien',
      apparition: -168.3,
      extinction: -166,
    },
    description:
      "Les spécialistes ne savent rien de ce dinosaure. Seul un fragment de mâchoire a été retrouvé. Il est même probable que ce dinosaure n'ait jamais existé, et donc, qu'il s'agit d'un autre dinosaure, voire même d'une autre espèce animale. Les mesures anatomiques ne sont donc que des estimations.",
    autres_infos: {
      taille: {
        longueur: '15 mètres',
        hauteur: '6 mètres',
      },
      poids: '20-30 tonnes',
    },
    decouverte: {
      date: '1988',
      lieu: 'Chine',
    },
  },
  {
    nom: 'Troodon',
    etymologie: 'Dent blessante',
    image_url: './assets/images/Troodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: 'Diapsida',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: 'Tetanurae',
      micro_ordre: '',
      super_famille: '',
      famille: 'Troodontidae',
      sous_famille: 'Troodontinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Troodon',
      espèces: 'Troodon formosus',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Campanien',
      apparition: -76.5,
      extinction: -75.5,
    },
    description:
      "Troodon est très connu médiatiquement pour son intelligence supposée en raison de la taille de sa cavité cérébrale par rapport à la taille de son corps, exceptionnellement grande pour un dinosaure, et de ses mains préhensiles à trois doigts avec des « pouces » partiellement opposables. <br /> Cependant, en 2017, il a été établi que cet animal ne serait connu qu'au travers d'une seule dent acérée et dentelée: les fossiles que l'on pensait être ceux de Troodon sont finalement ceux de deux autres dinosaures bien distincts. Cela dit, les débats sur l'existence de Troodon ne sont pas clos.",
    autres_infos: {
      taille: {
        longueur: '',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1855',
      lieu: 'Montana, États-Unis',
    },
  },
  {
    nom: 'Utahraptor',
    etymologie: "Voleur de l'Utah",
    image_url: './assets/images/Utahraptor.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: 'Coelurosauria',
      micro_ordre: '',
      super_famille: '',
      famille: 'Dromaeosauridae',
      sous_famille: 'Dromaeosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Utahraptor',
      espèces: 'Utahraptor ostrommaysi',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Valanginien',
      apparition: -135,
      extinction: -130,
    },
    description:
      "Utahraptor avait de grandes griffes recourbées sur leurs deuxièmes orteils. L'une de ces griffes retrouvées a une longueur de 22 centimètres. <br /> Il devait être agile et plutôt rapide. Il est possible qu'il devait avoir une carrure robuste, trapue et plus puissante que ces cousins Dromaeosauridae.",
    autres_infos: {
      taille: {
        longueur: '5-7 mètres',
        hauteur: '1.80 mètre',
      },
      poids: '280-500 kilogrammes',
    },
    decouverte: {
      date: '1975',
      lieu: 'Utah',
    },
  },
  {
    nom: 'Qijianglong',
    etymologie: 'Dragon du Qijiang',
    image_url: './assets/images/Qijianglong.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: 'Diapsida',
      infra_classe: 'Archosauromorpha',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Sauropodomorpha',
      infra_ordre: 'Sauropoda',
      micro_ordre: '',
      super_famille: '',
      famille: 'Mamenchisauridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Qijianglong',
      espèces: 'Qijianglong guokr',
    },
    regime_alimentaire: 'Végétarien',
    habitat: '',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Aptien',
      apparition: -115,
      extinction: -116,
    },
    description:
      "Les paléontologues sont en mesure de dire que Qijianglong possédait un crâne dont sa partie inférieure avait une plaque en forme de bosse. <br /> Les vertèbres du cou ont des prolongements qui raidissent le cou horizontalement mais permettent à l'animal tout de même un mouvement vertical.",
    autres_infos: {
      taille: {
        longueur: '15 mètres',
        hauteur: '7 mètres',
      },
      poids: '20-30 tonnes',
    },
    decouverte: {
      date: '1990',
      lieu: 'Chine',
    },
  },
  {
    nom: 'Paraceratherium',
    etymologie: "Près de l'animal sans corne",
    image_url: './assets/images/Paraceratherium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Perissodactyla',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Rhinocerotoidea',
      famille: 'Paraceratheriidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Paraceratherium',
      espèces:
        'Paraceratherium bugtiense, Paraceratherium huangheense, Paraceratherium linxiaense, Paraceratherium transouralicum',
    },
    regime_alimentaire: '',
    habitat: '',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Paléogène',
      epoque: 'Oligocène',
      stage: 'Rupélien',
      apparition: -37.2,
      extinction: -15.97,
    },
    description:
      "C'était l'un des plus grands mammifères terrestres de tous les temps. Il avait un corps massif, des jambes solides, un long cou et une tête semblable à celle d'un rhinocéros, mais sans corne. Il se nourrissait principalement de plantes. <br /> Paraceratherium est souvent considéré comme l'un des exemples les plus impressionnants de méga-faune préhistorique.",
    autres_infos: {
      taille: {
        longueur: '7-8 mètres',
        hauteur: '2-2.50 mètres',
      },
      poids: '11-20 tonnes',
    },
    decouverte: {
      date: '1846',
      lieu: 'Pakistan',
    },
  },
  {
    nom: 'Hesperornis',
    etymologie: "Oiseau d'Occident",
    image_url: './assets/images/Hesperornis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Hesperornithidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Hesperornis',
      espèces:
        'Hesperornis altus, Hesperornis bairdi, Hesperornis chowi, Hesperornis crassipes, Hesperornis gracilis, Hesperornis lumgairi, Hesperornis macdonaldi, Hesperornis mengeli, Hesperornis montana, Hesperornis regalis, Hesperornis rossicus',
    },
    regime_alimentaire: 'Piscivore',
    habitat: 'Falaises, roches de bord de mer',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Campanien',
      apparition: -83.6,
      extinction: -72,
    },
    description:
      "Cet animal ressemble à un oiseau. <br /> Son bec long est équipé de petites dents tranchantes simples, rangées dans une rainure longitudinale. <br /> Ses ailes très atrophiées ne lui permettaient pas de voler. <br /> Hesperornis était un excellent et puissant nageur et plongeur bien que rien n'indique que ses pattes étaient palmées. <br /> À cause de la morphologie de ses jambes, il devait être maladroit à terre où il passait peu de temps, sauf pour pondre et couver.",
    autres_infos: {
      taille: {
        longueur: '1.80',
        hauteur: '',
      },
      poids: '7-10 kilogrammes',
    },
    decouverte: {
      date: '1871',
      lieu: 'Kansas, États-Unis',
    },
  },
  {
    nom: 'Yawunik',
    etymologie: "Du nom d'un monstre marin de la mythologie Ktunaxa",
    image_url: './assets/images/Yawunik.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Megacheira',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Leanchoilida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Leanchoiliidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Yawunik',
      espèces: 'Yawunik kootenayi',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Miaolingien',
      stage: 'Wuliuen',
      apparition: -508,
      extinction: -507,
    },
    description:
      "Yawunik est un ancien animal marin doté de pattes articulées et d'un exosquelette fait de morceaux de carapaces en chitine. <br /> Son corps est divisé en douze segments, chacun ayant une patte biramée. <br /> Ces pattes lui servaient à nager et à transporter de la nourriture jusqu'à sa bouche. Sans organe masticateur, il broyait sa nourriture avec ses pattes postérieures, puis les pattes du tronc créaient un courant d'eau pour que les pattes céphaliques puissent la réceptionner et la digérer. <br /> À l'avant, Yawunik avait un bouclier céphalique avec des appendices frontaux très spécialisés, semblables à ceux des coléoptères. Ces appendices étaient munis de trois longues pinces, dont la troisième avait une griffe. <br /> Le long des deux autres pinces, il y avait des rangées de dents suivies de très longues tiges, qui avaient à la fois une fonction alimentaire et sensorielle, ce qui est unique. Ces appendices étaient bien adaptés pour capturer des proies, indiquant que le Yawunik était un prédateur actif. <br />Après avoir capturé sa proie, il devait probablement replier ces structures sous son corps pour la broyer. En tant que prédateur abondant, il jouait un rôle clé dans son écosystème en impactant la chaîne alimentaire.",
    autres_infos: {
      taille: {
        longueur: '12 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2015',
      lieu: 'Schistes de Burgess, Canada',
    },
  },
  {
    nom: 'Allosaurus',
    etymologie: 'Lézard étrange',
    image_url: './assets/images/Allosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Allosauridae',
      sous_famille: 'Allosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Allosaurus',
      espèces: 'Allosaurus europaeus, Allosaurus fragilis, Allosaurus jimmadseni',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme, forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Supérieur',
      stage: 'Kimméridgien',
      apparition: -155,
      extinction: -145,
    },
    description:
      "Allosaurus est l'un des dinosaures les plus connus car il est l'un des plus médiatisés. <br /> Il s'agissait d'un des plus grands prédateurs du Jurassique. Chassant possiblement en meute pour s'attaquer à des proies bien plus grosses que lui, il devait courir à environ 40km/h. <br /> Allosaurus avait un crâne relativement léger par rapport à sa taille corporelle. Son crâne était allongé et étroit, avec des orbites oculaires de forme ovale. <br /> Les narines étaient grandes, suggérant un bon odorat, tandis que les yeux étaient bien adaptés à la chasse. <br /> Ses dents étaient coniques, pointues, et légèrement recourbées vers l'arrière. Elles étaient aiguisées tranchantes et adaptées pour déchirer la chair de ses proies. <br /> Ses bras étaient relativement courts par rapport à sa taille et se terminaient par des mains à trois doigts, chacun doté de griffes acérées. Ces bras étaient peut-être utilisés pour saisir et tenir la proie.",
    autres_infos: {
      taille: {
        longueur: '8-12 mètres',
        hauteur: '3 mètres',
      },
      poids: '1.5-2 tonnes',
    },
    decouverte: {
      date: '1869',
      lieu: 'Colorado, États-Unis',
    },
  },
  {
    nom: 'Sarcosuchus',
    etymologie: 'Crocodile de chair',
    image_url: './assets/images/Sarcosuchus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: '',
      division: '',
      super_ordre: 'Crocodylomorpha',
      ordre: 'Crocodile',
      sous_ordre: 'Mesosuchia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Pholidosauridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Sarcosuchus',
      espèces: 'Sarcosuchus hartti, Sarcosuchus imperator',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Fleuves',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Hauterivien',
      apparition: -136.4,
      extinction: -93.5,
    },
    description:
      "On estime que ce crocodile atteignait sa taille adulte à l'âge de 50 ou 60 ans. <br /> Son museau est très allongé, et sa mâchoire comptait plus d'une centaine de dents. L'extrémité de son museau comprend une excroissance osseuse, qui n'a rien à voir avec la protubérance spongieuse des gavials mâles actuels. Il était couvert d'une carapace d'écussons osseux, qui portaient chacun des anneaux de croissance annuelle. <br /> Il devait chasser de la même manière que nos crocodiles actuels.",
    autres_infos: {
      taille: {
        longueur: '9-10 mètres',
        hauteur: '',
      },
      poids: '4 tonnes',
    },
    decouverte: {
      date: '1947',
      lieu: 'Algérie',
    },
  },
  {
    nom: 'Leptictidium',
    etymologie: 'Belette grâcieuse',
    image_url: './assets/images/Leptictidium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Eutheria',
      division: '',
      super_ordre: '',
      ordre: 'Leptictida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Pseudorhyncocyonidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Leptictidium',
      espèces:
        'Leptictidium auderiense, Leptictidium ginsburgi, Leptictidium listeri, Leptictidium nasutum, Leptictidium prouti, Leptictidium sigei, Leptictidium storchi, Leptictidium tobieni',
    },
    regime_alimentaire: 'Omnivore',
    habitat: 'Forêts',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Éocène',
      stage: 'Yprésien',
      apparition: -50,
      extinction: -35,
    },
    description:
      "Leptictidium est un animal avec une anatomie unique, mélangeant des caractéristiques primitives et des adaptations spécialisées. <br /> Il avait probablement un museau long et mobile. Sa dentition était petite par rapport à sa taille. <br /> On ne sait pas si Leptictidium se déplaçait en courant ou en sautant, mais il avait des pieds adaptés au saut.  Il avait de petites pattes avant avec des doigts courts et fragiles, et de grandes pattes arrière. Ses chevilles et articulations étaient assez flexibles. <br /> Des omparaisons avec d'autres espèces similaires suggèrent des modes de déplacement différents, mais des différences dans leurs squelettes rendent difficile de déterminer son mode de déplacement avec certitude.",
    autres_infos: {
      taille: {
        longueur: '60-90 centimètres',
        hauteur: '20 centimètres',
      },
      poids: '3 kilogrammes',
    },
    decouverte: {
      date: '1962',
      lieu: 'Allemagne',
    },
  },
  {
    nom: 'Ceratomyrmex',
    etymologie: 'Fourmi à corne',
    image_url: './assets/images/Ceratomyrmex.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: 'Bilateria',
      rameau: '',
      infra_règne: 'Protostomia',
      super_embranchement: 'Ecdysozoa',
      embranchement: 'Arthropoda',
      sous_embranchement: 'Hexapoda',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Insecta',
      sous_classe: 'Pterygota',
      infra_classe: 'Neoptera',
      division: '',
      super_ordre: 'Holometabola',
      ordre: 'Hymenoptera',
      sous_ordre: 'Apocrita',
      infra_ordre: 'Aculeata',
      micro_ordre: '',
      super_famille: 'Vespoidea',
      famille: 'Formicidae',
      sous_famille: 'Sphecomyrminae',
      tribu: 'Haidomyrmecini',
      sous_tribu: '',
      genre: 'Ceratomyrmex',
      espèces: 'Ceratomyrmex ellenbergeri',
    },
    regime_alimentaire: 'Carnivore',
    habitat: '',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Cénomanien',
      apparition: -98.8,
      extinction: -93.9,
    },
    description:
      "Découverte dans l'ambre de Birmanie, cette fourmi possède 'corne' entre ses deux antennes et est recouverte de longues soies sur toute sa partie antérieure, auxquelles s'ajoute une brosse d'épines sur la partie terminale spatulée. <br /> Les soies sur la corne fournissaient un coussin mou pour déplacer les nymphes et les larves dans un nid. Il est possible que les mâchoires aient été utilisées comme mécanisme de défense, pour éloigner les prédateurs ou étourdir les proies par une ouverture rapide des mandibules. <br /> La corne et les mandibules pouvaient être utilisées pour piéger des grosses proies, les soies fournissant des informations sensorielles sur la position de la proie et créant un frottement pour l'empêcher de bouger.",
    autres_infos: {
      taille: {
        longueur: '4-6 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2016',
      lieu: 'Birmanie',
    },
  },
  {
    nom: 'Kamuysaurus',
    etymologie: "Dérivant de la langue aïnou signifiant 'Lézard divin'",
    image_url: './assets/images/Kamuysaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Ornithischia',
      sous_ordre: 'Ornithopoda',
      infra_ordre: 'Iguanodontia',
      micro_ordre: '',
      super_famille: 'Hadrosauroidea',
      famille: 'Hadrosauridae',
      sous_famille: 'Saurolophinae',
      tribu: 'Edmontosaurini',
      sous_tribu: '',
      genre: 'Kamuysaurus',
      espèces: 'Kamuysaurus japonicus',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme, Régions côtières',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Maastrichtien',
      apparition: -72.1,
      extinction: -70.6,
    },
    description:
      "Ce dinosaure est le plus grand jamais découvert au Japon, bien qu'il existe des hadrosaures bien plus grands, ce qui suggère fortement que l'espèce était soumise au nanisme insulaire, fréquent sur les îles. <br /> Ce dinosaure était capable de se déplacer aussi bien à deux qu'à quatre pattes.",
    autres_infos: {
      taille: {
        longueur: '8 mètres',
        hauteur: '3 mètres',
      },
      poids: '4-5.5 tonnes',
    },
    decouverte: {
      date: '2003',
      lieu: 'Japon',
    },
  },
  {
    nom: 'Dollocaris',
    etymologie: '--',
    image_url: './assets/images/Dollocaris.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: 'Crustacea',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Thylacocephala',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Conchyliocarida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Dollocarididae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Dollocaris',
      espèces: 'Dollocaris ingens',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Moyen',
      stage: 'Callovien',
      apparition: -165,
      extinction: -163,
    },
    description:
      "Le corps de Dollocaris est recouvert d'une carapace, de laquelle sortent trois paires d'appendices préhensiles qui lui permettaient d'attraper ses proies, dont des crevettes (retrouvées dans son estomac). <br /> Les yeux hypertrophiés de Dollocaris ingens, en particulier, ont montré un nombre exceptionnel de 18 000 facettes par œil, nombre record pour un fossile. Cette caractéristique exceptionnelle indique que Dollocaris était un prédateur avec une vision probablement adaptée à un environnement marin faiblement éclairé.",
    autres_infos: {
      taille: {
        longueur: '5-20 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1923',
      lieu: 'Ardèche, France',
    },
  },
  {
    nom: 'Beelzebufo',
    etymologie: 'Crapaud de Belzébuth',
    image_url: './assets/images/Beelzebufo.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Amphibia',
      sous_classe: 'Lissamphibia',
      division: '',
      super_ordre: 'Salientia',
      ordre: 'Anura',
      sous_ordre: 'Neobatrachia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Ceratophryidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Beelzebufo',
      espèces: 'Beelzebufo ampinga',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Marécages',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Maastrichtien',
      apparition: -70,
      extinction: -66,
    },
    description:
      "La gueule de Beelzebufo était très grande et les os du sommet de son crâne portaient une surface externe rugueuse indiquant qu'une partie de sa tête était couverte de d'écailles osseuses appelées scutelles. <br /> Il n’est pas exclu que Beelzebufo ait pu se nourrir de lézards, de petits mammifères et de plus petites grenouilles, et même, vu sa taille, de dinosaures juvéniles. ",
    autres_infos: {
      taille: {
        longueur: '30-40 centimètes',
        hauteur: '10 centimètres',
      },
      poids: '4 kilogrammes',
    },
    decouverte: {
      date: '2008',
      lieu: 'Madagascar',
    },
  },
  {
    nom: 'Kunpengopterus',
    etymologie: '--',
    image_url: './assets/images/Kunpengopterus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: 'Tetrapoda',
      classe: '',
      sous_classe: 'Diapsida',
      infra_classe: 'Archosauromorpha',
      division: '',
      super_ordre: '',
      ordre: 'Pterosauria',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Wukongopteridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Kunpengopterus',
      espèces: 'Kunpengopterus antipollicatus, Kunpengopterus sinensis',
    },
    regime_alimentaire: 'Piscivore',
    habitat: '',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Supérieur',
      stage: 'Kimméridgien',
      apparition: -145,
      extinction: -145,
    },
    description:
      "Ce ptérosaure possédait des vertèbres cervicales allongées. <br /> Une crête osseuse basse est présente sur le crâne, juste derrière les yeux. Il n'y a cependant aucun signe de crête sur le museau ou de quille sous la mandibule. L'arrière du crâne est arrondi. Kunpengopterus a une longue queue raide. Le cinquième orteil est également long et fortement incurvé. <br /> L'espèce Kunpengopterus antipollicatus possède un doigt opposable ce qui est rarissime chez les animaux non-mammifères.",
    autres_infos: {
      taille: {
        longueur: '10 centimètres',
        hauteur: '',
        envergure: '20 centimètres',
      },
      poids: '',
    },
    decouverte: {
      date: '2010',
      lieu: 'Chine',
    },
  },
  {
    nom: 'Dracopristis',
    etymologie: '--',
    image_url: './assets/images/Dracopristis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Chondrichthyes',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Ctenacanthiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Dracopristis',
      espèces: 'Dracopristis hoffmanorum',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Carbonifère',
      epoque: 'Mississippien',
      stage: 'Serpukhovien',
      apparition: -330.9,
      extinction: -303.7,
    },
    description:
      "Ce requin avait 12 rangées de dents courtes et épaisses qui suggèrent qu'il était adapté pour saisir et écraser ses proies. <br /> Il possédait une série d'épines de près de 60 centimètres sur ses nageoires dorsales. <br /> La forme particulière des nageoires suggère que Dracopristis se cachait au fond de l'eau et qu'il utilisait ses épines pour se protéger des prédateurs. ",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2013',
      lieu: 'Nouveau-Mexique, États-Unis',
    },
  },
  {
    nom: 'Carnotaurus',
    etymologie: 'Taureau carnivore',
    image_url: './assets/images/Carnotaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: '',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Ceratosauria',
      famille: 'Abelisauridae',
      sous_famille: 'Carnotaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Carnotaurus',
      espèces: 'Carnotaurus sastrei',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Maastrichtien',
      apparition: -71,
      extinction: -69,
    },
    description:
      "Son nom provient de ses cornes de sa tête semblables à celles d'un taureau. <br /> Les yeux de Carnotaure étaient placés à l'avant de son crâne, ce qui est inhabituel pour un dinosaure et pourrait indiquer une vision binoculaire et une perception de profondeur. Cela étant un avantage non négligeable pour un prédateur. Ses bras sont minuscules (encore plus petits que ceux du Tyrannosaurus Rex). Ils ne devaient donc pas lui servir à agripper ses proies. <br /> Son crâne assez épais et solide contrastait avec sa mâchoire inférieure fine. Sa technique de chasse devait donc consister à repérer une proie (probablement de petite taille) à sa portée et se jeter la gueule ouverte sur celle-ci, en se servant de sa mâchoire supérieure pour les frapper comme une hache, sa vitesse servant à les rattraper.",
    autres_infos: {
      taille: {
        longueur: '7.5-9 mètres',
        hauteur: '3.5 mètres',
      },
      poids: '1-1.6 tonne',
    },
    decouverte: {
      date: '1984',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Llukalkan',
    etymologie: 'Celui qui fait peur, en langue mapuche',
    image_url: './assets/images/Llukalkan.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Abelisauridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Llukalkan',
      espèces: 'Llukalkan aliocranianus',
    },
    regime_alimentaire: 'Carnivore',
    habitat: '',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Santonien',
      apparition: -86,
      extinction: -83,
    },
    description:
      "Llukalkan est un dinosaure encore très peu connu, seulement grâce à son crâne. Celui-ci possède la particularité d'avoir un petit sinus postérieur rempli d'air dans la zone de l'oreille moyenne, qui n'a été trouvé chez aucun autre Abelisauridae. <br /> En raison de son oreille inhabituelle, on a supposé que Llukalkan avait un sens de l'ouïe plus aiguisé que les autres Abelisauridae.",
    autres_infos: {
      taille: {
        longueur: '6 mètres (?)',
        hauteur: '3 mètres (?)',
      },
      poids: '1-2 tonnes',
    },
    decouverte: {
      date: '2015',
      lieu: 'Argentine',
    },
  },
  {
    nom: 'Aquilolamna',
    etymologie: '--',
    image_url: './assets/images/Aquilolamna.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Chondrichthyes',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Lamniformes (?)',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Aquilolamnidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Aquilolamna',
      espèces: 'Aquilolamna milarcae',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Turonien',
      apparition: -93,
      extinction: -91,
    },
    description:
      "Cet animal aquatique a eu droit au surnom de 'requin-aigle' du fait de sa forme atypique. <br /> Sur la base d'un seul specimen connu, les paléontologues savent que Aquilolamna avait un corps et une queue en forme de torpille similaires à ceux de la plupart des requins, mais avait également une paire de nageoires pectorales extrêmement longues en forme d'ailes dont la largeur d'un bout à l'autre était plus large que longue. Celles-ci, combinés à sa tête large, ont conduit à l'hypothèse qu’Aquilolamna était un filtreur planctivore. Aquilolamna n'utilisait probablement pas ses nageoires pour se déplacer en 'volant' comme les raies manta actuelles.<br /> Il est plus plausible que ce requin avançait à l'aide de sa queue.",
    autres_infos: {
      taille: {
        longueur: '3 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2012',
      lieu: 'Mexique',
    },
  },
  {
    nom: 'Borealopelta',
    etymologie: 'Bouclier du nord',
    image_url: './assets/images/Borealopelta.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Ornithischia',
      sous_ordre: 'Thyreophora',
      infra_ordre: 'Ankylosauria',
      micro_ordre: '',
      super_famille: '',
      famille: 'Nodosauridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Borealopelta',
      espèces: 'Borealopelta markmitchelli',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Aptien',
      apparition: -125,
      extinction: -113,
    },
    description:
      "Le crâne de Borealopelta mesurait 36.3 centimètres de long et 33.5 centimètres de large. <br /> Les près de 200 ostéodermes présents sur son dos montrent qu'ils forment un « blindage » quasi-continu. Leurs formes sont très différentes, avec une taille variant de 5 à 30 centimètres de longueur. Ils sont disposés en de nombreuses rangées étroitement espacées, situées sur le dos et le flanc de l'animal. Sur chacune des épaules une de ces plaques s'est transformée en une longue épine ou pointe en forme de corne qui mesure plus de 55 centimètres de long. Le tout devait dissuader les prédateurs de s'attaquer à lui. ",
    autres_infos: {
      taille: {
        longueur: '5.5 mètres',
        hauteur: '1 mètre',
      },
      poids: '1.3 tonne',
    },
    decouverte: {
      date: '2011',
      lieu: 'Alberta, États-Unis',
    },
  },
  {
    nom: 'Callichimaaera',
    etymologie: 'Belle chimère',
    image_url: './assets/images/Callichimaaera.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Malacostraca',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Decapoda',
      sous_ordre: 'Pleocyemata',
      infra_ordre: 'Brachyura',
      micro_ordre: '',
      super_famille: 'Callichimaeroidea',
      famille: 'Callichimaeridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Callichimaera',
      espèces: 'Callichimaera perplexa',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Cénomanien',
      apparition: -95,
      extinction: -90,
    },
    description:
      "Callichimaera possédait des yeux composés, d'un petit corps fusiforme et de fines pièces buccales.<br />  Comme Callichimaera possède de grandes pattes en forme de palettes, il s'agissait probablement d'un nageur actif, même s'il vivait principalement sur le fond de la mer.",
    autres_infos: {
      taille: {
        longueur: '16 millimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2012',
      lieu: '',
    },
  },
  {
    nom: 'Pahvantia',
    etymologie: 'De la tribu Pahvant',
    image_url: './assets/images/Pahvantia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Dinocaridida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Radiodonta',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Hurdiidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Pahvantia',
      espèces: 'Pahvantia hastata',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Fond des océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Miaolingien',
      stage: 'Drumien',
      apparition: -509,
      extinction: -497,
    },
    description: 'Pahvantia capturait des proies plus grosses vivant le long ou dans les sédiments du fond des océans.',
    autres_infos: {
      taille: {
        longueur: '25-30 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1981',
      lieu: 'Utah',
    },
  },
  {
    nom: 'Drepanosaurus',
    etymologie: 'Lézard faucille',
    image_url: './assets/images/Drepanosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: 'Diapsida',
      infra_classe: 'Archosauromorpha',
      division: '',
      super_ordre: '',
      ordre: 'Prolacertiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Drepanosauridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Drepanosaurus',
      espèces: 'Drepanosaurus unguicaudatus',
    },
    regime_alimentaire: 'Insectivore',
    habitat: 'Forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Trias',
      epoque: 'Supérieur',
      stage: 'Rhétien',
      apparition: -208.5,
      extinction: -201.3,
    },
    description:
      "Bien qu'il manque de fragment de tête et de cou, Drepanosaurus passait probablement son temps dans les arbres, utilisant ses mains et ses pieds griffus pour grimper de branche en branche, ainsi que sa queue préhensile, à la manière d'un caméléon. <br /> La griffe située à l'extrémité de la queue pouvait également servir à agripper les branches.",
    autres_infos: {
      taille: {
        longueur: '45-50 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1979',
      lieu: 'Italie',
    },
  },
  {
    nom: 'Foreyia',
    etymologie: 'Hommage à Peter Lawrence Forey, paléontologue britannique spécialiste des poissons',
    image_url: './assets/images/Foreyia.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sarcopterygii',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Coelacanthiformes',
      sous_ordre: 'Latimerioidei',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Latimeriidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Foreyia',
      espèces: 'Foreyia maxkuhni',
    },
    regime_alimentaire: '',
    habitat: 'Océans',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Trias',
      epoque: 'Moyen',
      stage: 'Ladinien',
      apparition: -240,
      extinction: -237,
    },
    description:
      "L'apparence de ce possion est étrange: une tête proportionnellement énorme, une mâchoire courbé en forme de bec, une pointe en forme de pic. <br /> Trop peu connu, nous ne savons pratiquement rien de son mode de vie. ",
    autres_infos: {
      taille: {
        longueur: '15-25 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2017',
      lieu: 'Suisse',
    },
  },
  {
    nom: 'Capinatator',
    etymologie: 'Nageur saisissant',
    image_url: './assets/images/Capinatator.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chaetognatha',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Capinatator',
      espèces: 'Capinatator praetermissus',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Miaolingien',
      stage: 'Wuliuen',
      apparition: -509,
      extinction: -497,
    },
    description:
      "Capinatator est un ver qui a la particularité d'avoir 50 épines autour de sa bouche.<br />  Les épines servaient à saisir les proies pour les manger.",
    autres_infos: {
      taille: {
        longueur: '10 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2017',
      lieu: 'Colombie-Britanique',
    },
  },
  {
    nom: 'Saccorhytus',
    etymologie: 'Vase à boire',
    image_url: './assets/images/Saccorhytus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: 'Eumetazoa',
      rameau: '',
      infra_règne: '',
      super_embranchement: 'Ecdysozoa',
      embranchement: 'Saccorhytida',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Saccorhytidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Saccorhytus',
      espèces: 'Saccorhytus coronarius',
    },
    regime_alimentaire: '',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Terreneuvien',
      stage: 'Fortunien',
      apparition: -541,
      extinction: -529,
    },
    description:
      "Ce drôle d'animal avait un corps ovoïde muni d'une bouche proéminente entourée de deux rangées de protubérances. Saccorhytus devait recracher les déchets de leur digestion par la bouche car il semble avoir été dépourvu d'anus, et il ne possédait pas non plus d'yeux.<br />  La cuticule de ces animaux étaient épaisse, portant quelques cônes et petits pores circulaires, peut-être l'indication qu'il vivait en filtrant l'eau, dans le plancton ou dans les interstices entre les grains des sédiments marins.",
    autres_infos: {
      taille: {
        longueur: '1 millimètre',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2016',
      lieu: 'Chine',
    },
  },
  {
    nom: 'Tamisiocaris',
    etymologie: 'Crevette tamisée',
    image_url: './assets/images/Tamisiocaris.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Dinocaridida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Radiodonta',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Tamisiocarididae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Tamisiocaris',
      espèces: 'Tamisiocaris borealis',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Cambrien',
      epoque: 'Série 2',
      stage: 'Étage 3',
      apparition: -511,
      extinction: -514,
    },
    description:
      "Ce prédateur marin possédait une paire de tentacule à l'extrémité de sa tête se terminant par des fanions qui lui servaient sans doute à filtrer sa nourriture.",
    autres_infos: {
      taille: {
        longueur: '50-60 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2010',
      lieu: 'Groenland',
    },
  },
  {
    nom: 'Orthoceras',
    etymologie: 'Corne droite',
    image_url: './assets/images/Orthoceras.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Mollusca',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Cephalopoda',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Orthocerida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Orthoceratidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Orthoceras',
      espèces: 'Orthoceras regulare',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Ordovicien',
      epoque: 'Moyen',
      stage: 'Dapingien',
      apparition: -470,
      extinction: -458,
    },
    description:
      "Cet animal est un nautile préhistorique qui a la particularité de posséder une coquille en forme de cône droite, d'où son nom. <br /> Bien que son mode de vie exact reste en partie inconnu, il est raisonnable de penser que Orthoceras devait avoir le même mode de prédation que les céphalopodes marins actuels ou des pieuvres.",
    autres_infos: {
      taille: {
        longueur: '11 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1789',
      lieu: '',
    },
  },
  {
    nom: 'Spiclypeus',
    etymologie: 'Bouclier à épine',
    image_url: './assets/images/Spiclypeus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Ornithischia',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Ceratopsia',
      famille: 'Ceratopsidae',
      sous_famille: 'Chasmosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Spiclypeus',
      espèces: 'Spiclypeus shipporum',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Campanien',
      apparition: -76.24,
      extinction: -75.21,
    },
    description:
      "Spiclypeus se distingue des autres Chasmosaurinae par plusieurs caractéristiques uniques. <br /> Tout d'abord, il présente un contact de l'os nasal avec la surface latérale de la projection arrière du prémaxillaire. <br /> De plus, ses cornes de la cavité oculaire se dirigent vers le haut et sur les côtés. <br /> Les six épipariétales, qui sont des excroissances en forme de cornes sur la collerette, sont fusionnées à leur base. Les deux premières paires d'épipariétales s'enroulent autour de la face avant de la collerette, tandis que la troisième paire pointe vers l'arrière et vers le centre de la collerette.",
    autres_infos: {
      taille: {
        longueur: '4.5-6 mètres',
        hauteur: '3 mètres',
      },
      poids: '3-5 tonnes',
    },
    decouverte: {
      date: '2000',
      lieu: 'Montana, États-Unis',
    },
  },
  {
    nom: 'Machairoceratops',
    etymologie: 'Face cornue à épée courbée',
    image_url: './assets/images/Machairoceratops.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Ornithischia',
      sous_ordre: 'Ceratopsia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Ceratopsidae',
      sous_famille: 'Centrosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Machairoceratops',
      espèces: 'Machairoceratops cronusi',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Campanien',
      apparition: -80.6,
      extinction: -72.1,
    },
    description:
      "Ce dinosaure est caractérisé par sa collerette unique pour cératopsien: le haut de cette collerette se terminait comme la visière d'une casquette. <br /> Très peu connu et manquant d'informations pour cause de manque de fossiles, les paléontologues ne savent pas à quoi pouvait servir cet ornement particulier.",
    autres_infos: {
      taille: {
        longueur: '4-6 mètres',
        hauteur: '3 mètres',
      },
      poids: '3-4 tonnes',
    },
    decouverte: {
      date: '2006',
      lieu: 'Utah, États-Unis',
    },
  },
  {
    nom: 'Tullimonstrum',
    etymologie: 'Monstre du Tully',
    image_url: './assets/images/Tullimonstrum.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: 'Eumetazoa',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: '',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Tullimonstrum',
      espèces: 'Tullimonstrum gregarium',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Eaux très peu profondes',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Carbonifère',
      epoque: 'Pennsylvanien',
      stage: 'Moscovien',
      apparition: -311,
      extinction: -306,
    },
    description:
      "Tullimonstrum possédait une paire de nageoires ressemblant un peu à celle des seiches, peut-être également des nageoires verticales. <br /> Il possédait un long proboscis (plus simplement 'une trompe') pourvu de huit petites dents, avec lequel il cherchait probablement activement de petits animaux et des détritus comestibles dans les fonds vaseux. <br/><br/> En 2023, des scientifiques japonais ont réussi à prouver que Tullimonstrum était un animal invertébré. La classification taxonomique reste donc inconnue pour le moment.",
    autres_infos: {
      taille: {
        longueur: '10-35 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1955',
      lieu: 'Illinois, États-Unis',
    },
  },
  {
    nom: 'Lorrainosaurus',
    etymologie: 'Lézard de Lorraine',
    image_url: './assets/images/Lorrainosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Sauropterygia',
      ordre: 'Plesiosauria',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Pliosauroidea',
      famille: 'Pliosauridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Lorrainosaurus',
      espèces: 'Lorrainosaurus keileni',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Eaux peu profondes',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Moyen',
      stage: 'Aalénien',
      apparition: -174.1,
      extinction: -170.3,
    },
    description:
      "Baptisé Lorrainosaurus en 2023, ce prédateur possédait un crâne mesurant 1.30 mètre. <br /> Ce pliosaure est le plus ancien animal de ce groupe connu à ce jour. Il devait s'agir du plus grand prédateur marin de son temps, bien qu'il existera plus tard, des pliosaures bien plus massifs.",
    autres_infos: {
      taille: {
        longueur: '4-6 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1983',
      lieu: 'Lorraine, France',
    },
  },
  {
    nom: 'Mégalosaurus',
    etymologie: 'Lézard géant',
    image_url: './assets/images/Megalosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Megalosauria',
      famille: 'Megalosauridae',
      sous_famille: 'Megalosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Megalosaurus',
      espèces: 'Megalosaurus bucklandii',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassique',
      epoque: 'Moyen',
      stage: 'Bathonien',
      apparition: -168,
      extinction: -166,
    },
    description:
      "Mégalosaurus est le premier dinosaure à avoir été découvert en 1676, mais aussi le premier officiellement décrit par les paléontologues en 1824. <br /> C'était l'un des plus grands prédateurs du Jurassique. <br /> Bipède, il possédait une silhouette massive, des pattes puissantes et une tête dotée de dents acérées, faisant de lui un carnivore par excellence. Sa structure corporelle était adaptée à la chasse, avec des membres postérieurs robustes lui permettant de se déplacer rapidement pour capturer ses proies. <br /> Mégalosaurus présentait une combinaison de caractéristiques typiques des théropodes, telles que des bras courts et griffus, une queue puissante pour l'équilibre, et une tête allongée portant des dents conçues pour déchirer la chair de ses proies.",
    autres_infos: {
      taille: {
        longueur: '6-7 mètres',
        hauteur: '3 mètres',
      },
      poids: '2 tonnes',
    },
    decouverte: {
      date: '1676',
      lieu: 'Angleterre',
    },
  },
  {
    nom: 'Qantassaurus',
    etymologie: 'Lézard de Qantas',
    image_url: './assets/images/Qantassaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Ornithischia',
      sous_ordre: 'Ornithopoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Hypsilophodontidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Qantassaurus',
      espèces: 'Qantassaurus intrepidus',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Albien',
      apparition: -125,
      extinction: -112,
    },
    description:
      "Ce petit dinosaure possédait une tête courte et allongée. <br /> Bipède, ses pattes avant étaient pourvues de 5 doigts tandis que les pattes arrières n'en possédaient que 4. Il se caractérisait aussi par une queue aussi longue que son corps et sa tête réunis. <br /> On suppose que sa morphologie lui donnait une bonne aisance à la course.",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '1 mètre',
      },
      poids: '20-30 kilogrammes',
    },
    decouverte: {
      date: '1996',
      lieu: 'Australie',
    },
  },
  {
    nom: 'Nanuqsaurus',
    etymologie: 'Lézard ours blanc',
    image_url: './assets/images/Nanuqsaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Tyrannosauroidea',
      famille: 'Tyrannosauridae',
      sous_famille: 'Tyrannosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Nanuqsaurus',
      espèces: 'Nanuqsaurus hoglundi',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Neige',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Campanien',
      apparition: -70,
      extinction: -68,
    },
    description:
      "Le crâne de Nanuqsaurus mesurait entre 60 et 70 centimètres. La forme de son crâne suggère que son cerveau avait une zone particulièrement développée consacrée à l'odorat, ce qui semble indiquer que l'animal se fiait beaucoup aux odeurs pour chasser ses proies. <br /> En raison de sa taille et du climat, Nanuqsaurus devait être couvert d'une épaisse couche de plumes duveteuses.",
    autres_infos: {
      taille: {
        longueur: '6-7 mètres',
        hauteur: '2 mètres',
      },
      poids: '1-1.5 tonne',
    },
    decouverte: {
      date: '2006',
      lieu: 'Alaska',
    },
  },
  {
    nom: 'Méganeuropsis',
    etymologie: 'Ailes géantes à nervures',
    image_url: './assets/images/Meganeuropsis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: 'Hexapoda',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Insecta',
      sous_classe: '',
      division: '',
      super_ordre: 'Odonatoptera',
      ordre: 'Meganisoptera',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Meganeuridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Meganeuropsis',
      espèces: 'Meganeuropsis americana, Meganeuropsis permiana',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Marais forestiers',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Permien',
      epoque: 'Cisuralien',
      stage: 'Artinskien',
      apparition: -290.1,
      extinction: -283.5,
    },
    description:
      "Il s'agit de la plus grande libellule de tous les temps, dépassant de très peu la libellule Méganeura du Carbonifère. <br /> Méganeuropsis est caractérisé par ses ailes d'une envergure incroyable. Il y a fort à parier que l'on pouvait l'entendre venir de loin. <br /> Chassant dans les endroits forestiers et à proximité de plans d'eau, il se nourissait de petit lézards existants à son époque.",
    autres_infos: {
      taille: {
        longueur: '33 centimètres',
        hauteur: '',
        envergure: '71 centimètres',
      },
      poids: '170 grammes',
    },
    decouverte: {
      date: '1937',
      lieu: 'Kansas, États-Unis',
    },
  },

  {
    nom: 'Stegosaurus',
    etymologie: 'Lézard toité',
    image_url: './assets/images/Stegosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Reptilia',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Ornithischia',
      sous_ordre: 'Thyreophora',
      infra_ordre: 'Stegosauria',
      micro_ordre: '',
      super_famille: '',
      famille: 'Stegosauridae',
      sous_famille: 'Stegosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Stegosaurus',
      espèces:
        'Stegosaurus armatus, Stegosaurus duplex, Stegosaurus stenops, Stegosaurus sulcatus, Stegosaurus ungulatus',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Plaines, forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Jurassqiue',
      epoque: 'Supérieur',
      stage: 'Kimméridgien',
      apparition: -155,
      extinction: -152,
    },
    description:
      "L'un des dinosaure les plus connus qui soit, Stegosaurus était très facilement identifiable à ses 17 plaques osseuses disposées en deux rangées de sur le dos, plaques de formes et de tailles différentes selon les espèces. Des vaisseaux sanguins, encore visibles sur les fossiles, irriguaient ces plaques : il est probable qu'elles rougissaient avec l'afflux sanguin et probablement une vasodilatation. Il est possible que ces plaques aient été dissuasives contre les prédateurs les informant du danger, et également un atout nuptial. <br />Stegosaurus possédait une queue caractéristique car elle présentait à son extrémité quatre pointes osseuses, dont la longueur pouvait atteindre le mètre. Compte tenu de la position de sa tête, ce dinosaure devait se nourrir de plantes proches du sol. Son poids, sa morphologie osseuse et les suppositions quant aux potentielles attaches musculaires faisaient probablement de lui un herbivore placide, incapable de courir. <br />De plus, le cerveau de cet animal n'était pas plus gros qu'une noix.",
    autres_infos: {
      taille: {
        longueur: '7-12 mètres',
        hauteur: '1.50 mètre',
      },
      poids: '4 tonnes',
    },
    decouverte: {
      date: '1878',
      lieu: 'Colorado, États-Unis',
    },
  },
  {
    nom: 'Iguanodon',
    etymologie: "Dent d'iguane",
    image_url: './assets/images/Iguanodon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: 'Diapsida',
      infra_classe: 'Archosauromorpha',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Ornithischia',
      sous_ordre: 'Ornithopoda',
      infra_ordre: 'Iguanodontia',
      micro_ordre: '',
      super_famille: '',
      famille: 'Iguanodontidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Iguanodon',
      espèces: 'Iguanodon anglicus, Iguanodon bernissartensis, Iguanodon galvensis',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Barrémien',
      apparition: -126,
      extinction: -122,
    },
    description:
      "Iguanodon est le deuxième dinosaure de l'Histoire a avoir été décrit et nommé. <br /> Ce dinosaure était à la fois bipède et quadrupède. Il avait un corps massif. On a retrouvé des empreintes fossilisées avec la trace des quatre pattes. <br /> Cependant, la plupart ne révèlent que les empreintes des membres postérieurs, et on suppose que les iguanodons juvéniles avaient les bras trop courts pour s’en servir lors de leurs déplacements. <br /> L'iguanodon est caractérisé par un pouce très pointu qui pouvait servir à poignarder les carnivores le menaçant. Lors de la découverte de ce pouce, les paléontologues ont d'abord cru qu'il s'agissait d'une corne. <br /> Iguanodon était équipé d'un bec corné qui servait à arracher la végétation avec l'aide d'une langue préhensile, et ses dents aplaties lui permettaient de mâcher sa nourriture.",
    autres_infos: {
      taille: {
        longueur: '6-10 mètres',
        hauteur: '5 mètres',
      },
      poids: '4 tonnes',
    },
    decouverte: {
      date: '1825',
      lieu: 'Angleterre',
    },
  },
  {
    nom: 'Aegirocassis',
    etymologie: '--',
    image_url: './assets/images/Aegirocassis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Arthropoda',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Dinocaridida',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Radiodonta',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Anomalocarida',
      sous_famille: 'Aegirocassisinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Aegirocassis',
      espèces: 'Aegirocassis benmoulai',
    },
    regime_alimentaire: 'Filtreur',
    habitat: 'Océans',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Ordovicien',
      epoque: 'Inférieur',
      stage: 'Trémadocien',
      apparition: -480,
      extinction: -475,
    },
    description:
      "Aegirocassis possède une tête en forme d'épée pointant vers l'avant, ce qui devait faire de lui un animal rapide à la nage. Les pièces buccales comportaient deux appendices pourvus de longues soies permettant de filtrer de grandes quantités d'eau de mer pour en récupérer le plancton.<br />  Derrière la bouche, une sorte de poche permettait de stocker la nourriture pour l'ingérer plus tard.<br /> Aegirocassis possède aussi des yeux composés.",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2015',
      lieu: 'Maroc',
    },
  },
  {
    nom: 'Hipparion',
    etymologie: 'Petit poney',
    image_url: './assets/images/Hipparion.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Perissodactyla',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Equidae',
      sous_famille: 'Equinae',
      tribu: 'Hipparionini',
      sous_tribu: '',
      genre: 'Hipparion',
      espèces:
        'Hipparion chiai, Hipparion concudense, Hipparion condoni, Hipparion crassum, Hipparion dietrichi, Hipparion fissurae, Hipparion forcei, Hipparion gromovae, Hipparion laromae, Hipparion longipes, Hipparion lufengense, Hipparion macedonicum, Hipparion matthewi, Hipparion mediterraneum, Hipparion molayanense, Hipparion minus, Hipparion periafricanum, Hipparion philippus, Hipparion phlegrae, Hipparion prostylum, Hipparion rocinantis, Hipparion sellardsi, Hipparion shirleyae, Hipparion sithonis, Hipparion sitifense, Hipparion tehonense, Hipparion theniusi, Hipparion venustum',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Plaines',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Aquitanien',
      apparition: -23,
      extinction: -0.781,
    },
    description:
      "Hipparion était l'un des animaux herbivores les plus répandus de son temps. <br /> Hipparion ressemblait beaucoup à un cheval actuel, mais possédait trois doigts, dont le doigt médian était le plus développé.<br /> Parmi les différences entre l'hipparion et le cheval actuel, le premier possédait un os trapèze et un rudiment de cinquième os métacarpien, dont le deuxième est dépourvu.<br /> Ses dents possèdent de l'émail.<br /> Le crâne présente des fossettes préorbitales avec un développement différent selon les espèces, et on connaît quelques exemples dont la forme des os du nez peut faire penser à la présence d'une courte trompe.",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '1.40 mètres',
      },
      poids: '200 kilogrammes',
    },
    decouverte: {
      date: '1832',
      lieu: 'Autour du globe',
    },
  },
  {
    nom: 'Mégantereon',
    etymologie: '--',
    image_url: './assets/images/Megantereon.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Carnivora',
      sous_ordre: 'Feliformia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Felidae',
      sous_famille: 'Machairodontinae',
      tribu: 'Smilodontini',
      sous_tribu: '',
      genre: 'Megantereon',
      espèces:
        'Megantereon cultridens, Megantereon adroveri, Megantereon ekidoit, Megantereon falconeri, Megantereon hesperus, Megantereon microta, Megantereon vakhshensis, Megantereon whitei',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Plaines',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Messinien',
      apparition: -6,
      extinction: -0.781,
    },
    description:
      "Mégantereon devait ressembler à un grand jaguar moderne mais en plus massif. <br /> Il possèdait des membres antérieurs trapus, dont la moitié inférieure est de la taille d'un lion. Les muscles de son cou sont larges et conçus pour donner une puissante morsure de cisaillement. Les canines supérieures allongées étaient protégées par des rebords au niveau de la mâchoire.",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '72-75 centimètres',
      },
      poids: '90-150 kilogrammes',
    },
    decouverte: {
      date: '1828',
      lieu: 'Autour du globe',
    },
  },
  {
    nom: 'Haboroteuthis',
    etymologie: '--',
    image_url: './assets/images/Haboroteuthis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Mollusca',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Cephalopoda',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Teuthida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Haboroteuthis',
      espèces: 'Haboroteuthis poseidon',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Santonien',
      apparition: -86.3,
      extinction: -83.6,
    },
    description:
      'Assez peu connu des paléontologues, nous ne savons pas grand chose sur cet animal ressamblant à un calmar géant. <br /> Sa mâchoire est cependant très impressionnante par rappot à sa taille. Haboroteuthis possédait des dents mesurant de un à trois centimètres de large.<br /> Il possédait aussi une crête de 6 centimètres.',
    autres_infos: {
      taille: {
        longueur: '2 mètres (?)',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2015',
      lieu: 'Japon',
    },
  },
  {
    nom: 'Nanaimoteuthis',
    etymologie: '--',
    image_url: './assets/images/Nanaimoteuthis.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Mollusca',
      sous_embranchement: '',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Cephalopoda',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Teuthida',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: '',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Nanaimoteuthis',
      espèces: 'Nanaimoteuthis hikidai',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Santonien',
      apparition: -86.3,
      extinction: -83.6,
    },
    description: "Encore très mal-connue, cette créature devait ressembler au vampire des abysses d'aujourd'hui.",
    autres_infos: {
      taille: {
        longueur: '60 centimètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2015',
      lieu: 'Japon',
    },
  },
  {
    nom: 'Purgatorius',
    etymologie: 'De la Colline du Purgatoire, lieu de sa découverte',
    image_url: './assets/images/Purgatorius.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Plesiadapiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Purgatoriidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Purgatorius',
      espèces:
        'Purgatorius ceratops, Purgatorius coracis, Purgatorius janisae, Purgatorius mckeeveri, Purgatorius pinecreeensis, Purgatorius titusi, Purgatorius unio',
    },
    regime_alimentaire: 'Omnivore',
    habitat: 'Terre ferme, forêts',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Paléogène',
      epoque: 'Paléocène',
      stage: 'Danien',
      apparition: -66,
      extinction: -63,
    },
    description:
      "Purgatorius reste mal connu du fait de la rareté des fossiles découverts à ce jour. Seuls quelques fragments de mâchoire et des dents isolées (canines et molaires) ont été exhumés. Purgatorius avait probablement le même aspect qu'un rat ou un petit écureuil. <br /> Grâce à ses mœurs semi-arboricoles, permises par des chevilles très mobiles, il devait être aussi à l'aise dans les arbres que sur la terre ferme.",
    autres_infos: {
      taille: {
        longueur: '15 centimètres',
        hauteur: '4 centimètres',
      },
      poids: '100 grammes',
    },
    decouverte: {
      date: '1965',
      lieu: 'Montana, États-Unis',
    },
  },
  {
    nom: 'Eusthenopteron',
    etymologie: 'Aux bonnes vigoureuses nageoires',
    image_url: './assets/images/Eusthenopteron.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sarcopterygii',
      sous_classe: 'Tetrapodomorpha',
      division: '',
      super_ordre: '',
      ordre: '',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Tristichopteridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Eusthenopteron',
      espèces:
        'Eusthenopteron foordi, Eusthenopteron farloviensis, Eusthenopteron jenkinsi, Eusthenopteron obruchevi, Eusthenopteron savesoderberghi, Eusthenopteron traquairi, Eusthenopteron wenjukowi',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Rivières',
    geologie: {
      ere: 'Paléozoïque',
      periode: 'Dévonien',
      epoque: 'Moyen',
      stage: 'Givétien',
      apparition: -385,
      extinction: -382,
    },
    description:
      "Eusthenopteron possédait des dents fines servant plus à maintenir une proie plutôt qu'à la tuer. <br /> Il possède des narines internes, appelées choanes, une caractéristique que l'on retrouve seulement chez les animaux terrestres et chez les sarcoptérygiens, ce qui peut indiquer qu'Eusthenopteron pouvait respirer aussi bien dans l'eau qu'à l'air libre, ce qui sous-entend aussi la présence de poumons. Sa queue était caractéristique, puisque son profil était courbé et s'achevait en trois pointes, à la manière d'un trident. <br /> La nageoire postérieure a dû fournir à l'animal la capacité de réaliser des élans rapides, indispensables pour la chasse et pour la fuite.",
    autres_infos: {
      taille: {
        longueur: '1.20-2.10 mètre',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1881',
      lieu: 'Québec',
    },
  },
  {
    nom: 'Nigersaurus',
    etymologie: 'Lézard du Niger',
    image_url: './assets/images/Nigersaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Sauropodomorpha',
      infra_ordre: 'Sauropoda',
      micro_ordre: '',
      super_famille: 'Diplodocoidea',
      famille: 'Rebbachisauridae',
      sous_famille: 'Rebbachisaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Nigersaurus',
      espèces: 'Nigersaurus taqueti',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Terre ferme',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Inférieur',
      stage: 'Aptien',
      apparition: -115,
      extinction: -110,
    },
    description:
      "Ce dinosaure herbivore avait une tête en forme de brosse d'aspirateur et il possédait 500 dents en forme de clou. Une aussi grande quantité de dents est unique chez les sauropodes et n'avait auparavant été découverte que chez des hadrosaures ou des cératopsidés. <br /> Il avait un cou plus court que certains autres sauropodes. Il ne pouvait pas lever la tête plus haut que le niveau de son dos, à cause de ses os trop fins et légers. <br /> Il devait se nourrir comme les vaches actuelles en broutant les buissons ou branches au sol.",
    autres_infos: {
      taille: {
        longueur: '15 mètres',
        hauteur: '1.30-2 mètres',
      },
      poids: '4 tonnes',
    },
    decouverte: {
      date: '1976',
      lieu: 'Niger',
    },
  },
  {
    nom: 'Yaguarasaurus',
    etymologie: 'Lézard de Yaguará',
    image_url: './assets/images/Yaguarasaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Squamata',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Mosasauroidea',
      famille: 'Mosasauridae',
      sous_famille: 'Yaguarasaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Yaguarasaurus',
      espèces: 'Yaguarasaurus columbianus, Yaguarasaurus regiomontanus',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Mers',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Turonien',
      apparition: -93.9,
      extinction: -89.8,
    },
    description:
      "Les restes découverts (un crâne articulé, quelques vertèbres et côtes) ont été définis comme un nouveau genre et une nouvelle espèce de mosasauridé. Les premiers fossiles de cet animal suggèrent une longueur crânienne de 47 centimètres. Il a été découvert plus tard des crânes long de 87 centimètres. <br /> L'espèce Yaguarasaurus regiomontanus possède la particularité d'avoir une langue fourchue, ce qui est exceptionnel",
    autres_infos: {
      taille: {
        longueur: '5 mètres',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '1957',
      lieu: 'Colombie',
    },
  },
  {
    nom: "Lion de l'Atlas",
    etymologie: '--',
    image_url: './assets/images/Lion_Atlas.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: 'Theria',
      infra_classe: 'Eutheria',
      division: '',
      super_ordre: '',
      ordre: 'Carnivora',
      sous_ordre: 'Feliformia',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Felidae',
      sous_famille: 'Pantherinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Panthera',
      espèces: 'Panthera leo leo',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Montagnes marocaines',
    geologie: {
      ere: 'Cénozoïque',
      periode: '',
      epoque: '',
      stage: '',
      apparition: '',
      extinction: 1942,
    },
    description:
      "Le lion de l'Atlas, aussi appelé Lion de Barbarie, est caractérisé par une crinière beaucoup plus volumineuse que celle de ses cousins subsahariens, très sombre voire noire et allant jusqu'au milieu du ventre. Cette crinière serait due à son environnement plus tempéré. Son crâne mesure entre 30,85 cm et 37,23 cm de long. <br /> La durée de gestation des lionnes serait de 100 à 116 jours, avec 2 à 4 petits par portée. Les lions de l'Atlas auraient une longévité de 30 ans. Les Romains utilisaient des lions de Barbarie dans leurs amphithéâtres pour les combats de gladiateurs. <br /> Au Moyen Âge, les lions conservés dans la ménagerie de la Tour de Londres étaient des lions de Barbarie, preuve apportée par des tests ADN sur les deux crânes bien conservés dans la tour en 1937. <br /> La datation au carbone 14 a évalué les crânes aux environs de 1280-1385 pour l'un et 1420-1480 pour l'autre. Des fossiles appartenant au Lion de l'Atlas datant d'entre 100 000 à 110 000 ans ont été découverts dans la grotte de Bizmoune, près d'Essaouira. <br /> Le Dr Nobuyuki Yamaguchi, de l'Unité de conservation de la faune à l'Université d'Oxford, a indiqué que la croissance des civilisations le long du Nil et dans la péninsule du Sinaï au début du IIe millénaire av. J.-C., avait ainsi isolé les populations de lions. Jusqu’à il y a environ 100 ans, le lion a survécu à l'état sauvage au nord-ouest de l'Afrique, zone correspondant à l'Algérie, à la Tunisie et au Maroc. Le dernier spécimen sauvage fut vraisemblablement abattu en 1942 à Taddert dans l'Atlas marocain (versant nord du Tizi n'Tichka) ou aurait disparu lors de la destruction des forêts au nord de Sétif en 1958.",
    autres_infos: {
      taille: {
        longueur: '2.35-2.80 mètres',
        hauteur: '1.20 mètre',
      },
      poids: '270-300 kilogrammes',
    },
    decouverte: {
      date: '',
      lieu: '',
    },
  },
  {
    nom: 'Tyrannosaurus',
    etymologie: 'Lézard tyran',
    image_url: './assets/images/Tyrannosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: 'Tetanurae',
      micro_ordre: 'Coelurosauria',
      super_famille: 'Tyrannosauroidea',
      famille: 'Tyrannosauridae',
      sous_famille: 'Tyrannosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Tyrannosaurus',
      espèces: 'Tyrannosaurus mcraeensis, Tyrannosaurus rex',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Terre ferme, forêts',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Campanien',
      apparition: -71,
      extinction: -66,
    },
    description:
      "D'abord appelé 'Dynamosaurus imperiosus' lors de sa découverte, Tyrannosaurus a été l'un des plus grands ayant vécu sur Terre et surtout le plus célèbre. <br /> Son crâne mesurant près de 1.5 mètre possédait de larges cavités aériennes qui permettaient de réduire la masse du crâne, et laissaient la place aux attaches des muscles de la mâchoire, comme chez tous les théropodes carnivores. <br /> Mais, sur d'autres aspects, le crâne de Tyrannosaurus est significativement différent de celui des autres grands théropodes. Extrêmement large à l'arrière et muni d'un museau étroit, il permet une très bonne vision stéréoscopique.<br /> Sa mâchoire lui permettait d'arracher une des dizaines de kilos de chair et d'os en une bouchée. Selon une étude publiée en 2012, elle pouvait appliquer sur une seule dent postérieure une force estimée entre 35 000 et 57 000 N, soit une pression équivalente d'environ 3,5 à 5,7 tonnes, capable de broyer une voiture. <br /> Les prémaxillaires à l'avant de la mâchoire supérieure sont resserrés, avec des crêtes de renforcement sur la surface arrière, en forme d'incisive recourbée vers l'arrière, réduisant ainsi le risque que les dents ne s'arrachent quand Tyrannosaurus mordait et tirait. Les autres dents sont robustes, plus largement espacées et également renforcées par des crêtes. Les dents de la mâchoire supérieure sont plus grandes que toutes les autres. La plus grande trouvée à ce jour est estimée à 30 centimètres de long, racine comprise, ce qui en fait la plus grande dent de dinosaure carnivore. Quant à ses bras d'un mètre, ridiculement petits pour un animal de cette taille, leur structure osseuse particulièrement solide, la force considérable développée par leurs muscles et leur amplitude de mouvement limitée, pourraient indiquer un système anatomique conçu pour agripper vite et fort une proie se débattant. <br /> Durant des années, on pensé que le dinosaure Nanotyrannus était une espèce différente de dinosaure théropode carnivore. Ce n'est qu'en 2020 que les paléontologues ont compris qu'il s'agissait en réalité de Tyrannosaurus juvénile.<br /> En 2024 a été découvert Tyrannosaurus mcraeensis, un dinosaure légèrement plus ancien que le T-Rex.",
    autres_infos: {
      taille: {
        longueur: '10-13 mètres',
        hauteur: '4-6 mètres',
      },
      poids: '8-10 tonnes',
    },
    decouverte: {
      date: '1874',
      lieu: 'Colorado, États-Unis',
    },
  },
  {
    nom: 'Synthetoceras',
    etymologie: '--',
    image_url: './assets/images/Synthetoceras.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      infra_classe: '	Placentalia',
      division: '',
      super_ordre: 'Laurasiatheria',
      ordre: 'Cetartiodactyla',
      sous_ordre: 'Tylopoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Protoceratidae',
      sous_famille: 'Synthetoceratinae',
      tribu: '',
      sous_tribu: '',
      genre: '',
      espèces: 'Synthetoceras davisorum, Synthetoceras tricornatus',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Forêts',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Néogène',
      epoque: 'Miocène',
      stage: 'Tortonien',
      apparition: -10.6,
      extinction: -5.3,
    },
    description:
      "Ce dernier représentant des Protoceratidae était aussi le plus grand. Synthetoceras ressemblait à un cerf.<br />  Il possédait un crâne long et bas, équipé de deux cornes au-dessus de ses yeux, qui semblaient assez normales et similaires à celles de nombreux mammifères à cornes modernes. <br /> Les mâles étaient dotés d'une longue corne ramifiée en forme de 'Y' sur le museau qui leur servait probablement dans des combats territoriaux.",
    autres_infos: {
      taille: {
        longueur: '2 mètres',
        hauteur: '1 mètre',
      },
      poids: '150-200 kilogrammes',
    },
    decouverte: {
      date: '1932',
      lieu: 'Texas, États-Unis',
    },
  },
  {
    nom: 'Kwatisuchus',
    etymologie: "Crocodile à museau pointu. Du tupi 'Kwati' qui signifie « museau pointu »",
    image_url: './assets/images/Kwatisuchus.jpeg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Amphibia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Temnospondyli',
      sous_ordre: 'Stereospondyli',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Benthosuchidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: '',
      espèces: 'Kwatisuchus rosai',
    },
    regime_alimentaire: 'Carnivore',
    habitat: 'Rivières',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Trias',
      epoque: 'Inférieur',
      stage: 'Olénékien',
      apparition: -250,
      extinction: -250,
    },
    description:
      "Tout récemment découvert, les spécialistes manquent d'informations quand à son mode de vie exact. Par contre, grâce à cette découverte, les scientifiques ont eu la surprise de découvrir que les parents les plus proches de Kwatisuchus se trouvaient dans l'actuelle Russie, ce qui démontre qu'à un moment de l'histoire de la Terre, la faune du Brésil et de la Russie avaient une connexion, donc les deux régions se touchaient.",
    autres_infos: {
      taille: {
        longueur: '1.50 mètre',
        hauteur: '',
      },
      poids: '',
    },
    decouverte: {
      date: '2022',
      lieu: 'Rio Grande do Sul, Brésil',
    },
  },

  {
    nom: 'Eoneophron',
    etymologie: 'Nouveau Neophron',
    image_url: './assets/images/Eoneophron.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: '',
      sous_classe: '',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Saurischia',
      sous_ordre: 'Theropoda',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Caenagnathidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Eoneophron',
      espèces: 'Eoneophron infernalis',
    },
    regime_alimentaire: 'Ovivore',
    habitat: 'Forêts (?)',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Maastrichtien',
      apparition: -68,
      extinction: -66,
    },
    description:
      "Seul le fémur de cette nouvelle espèce d'oviraptor a été découvert. D’après l’analyse histologique, (étude de la structure microscopique des tissus animaux et végétaux et des cellules qui les composent) cet oviraptor était un jeune adulte, il n’avait donc pas encore atteint sa taille maximale. Il est donc possible d'avoir affaire à un animal plus grand que ce que l'on pense.",
    autres_infos: {
      taille: {
        longueur: '',
        hauteur: '',
      },
      poids: 'environ 80 kilogrammes',
    },
    decouverte: {
      date: '2024',
      lieu: 'Hell Creek, South Dakota, États-Unis',
    },
  },
  {
    nom: 'Elasmosaurus',
    etymologie: 'Reptile à plaque mince',
    image_url: './assets/images/Elasmosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Sauropsida',
      sous_classe: '',
      division: '',
      super_ordre: 'Sauropterygia',
      ordre: 'Plesiosauria',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: 'Plesiosauroidea',
      famille: 'Elasmosauridae',
      sous_famille: 'Elasmosaurinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Elasmosaurus',
      espèces: 'Elasmosaurus platyurus',
    },
    regime_alimentaire: 'Piscivore',
    habitat: 'Océans',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Campanien',
      apparition: -80.5,
      extinction: -66,
    },
    description:
      "Elasmosaurus était un plésiosaure marin avec un cou extrêmement long, composé de 71 vertèbres cervicales. Bien que son cou soit très long, il ne pouvait pas le recourber comme un serpent. Sa taille exacte n'est pas connue, mais il est considéré comme l'un des plus grands plésiosaures ayant jamais existé. <br /> <br /> Son corps était fuselé et ses membres étaient en forme de nageoires, adaptés à la vie aquatique. Ses vertèbres caudales étaient presque circulaires, avec des facettes costales sur la partie inférieure. Ses membres en forme de nageoires étaient probablement utilisés pour la propulsion, mais sans capacité de rotation.  <br /><br /> Elasmosaurus avait une tête triangulaire et des dents en forme de crocs pour capturer ses proies. Il chassait probablement en s'approchant discrètement de ses proies depuis les profondeurs, utilisant son long cou pour les saisir. <br /> <br /> Ce géant des mers donnait naissance à des petits vivants, comme d'autres reptiles marins, plutôt que de pondre des œufs sur la terre ferme.",
    autres_infos: {
      taille: {
        longueur: '10 mètres',
        hauteur: '',
      },
      poids: '2-9 tonnes',
    },
    decouverte: {
      date: '1867',
      lieu: 'Kansas, États-Unis',
    },
  },
  {
    nom: 'Ankylosaurus',
    etymologie: 'Lézard rigide',
    image_url: './assets/images/Ankylosaurus.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: 'Tetrapoda',
      classe: 'Sauropsida',
      sous_classe: 'Diapsida',
      division: '',
      super_ordre: 'Dinosauria',
      ordre: 'Ornithischia',
      sous_ordre: 'Thyreophora',
      infra_ordre: 'Ankylosauria',
      micro_ordre: '',
      super_famille: '',
      famille: 'Ankylosauridae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Ankylosaurus',
      espèces: 'Ankylosaurus magniventris',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Forêts, plaines',
    geologie: {
      ere: 'Mésozoïque',
      periode: 'Crétacé',
      epoque: 'Supérieur',
      stage: 'Maastrichtien',
      apparition: -68,
      extinction: -66,
    },
    description:
      "Ankylosaurus était un dinosaure recouvert d'une épaisse armure osseuse sur tout son corps. Cet animal est surtout connu pour sa queue massive qui se terminait pas une massue suffisament puissante qui pouvait briser les os de ses prédateurs, comme le célèbrissime T-Rex. Lorsqu'il était attaqué, il pouvait s'aplatir sur le sol et replier ses pattes pour protéger son ventre et donc, ses organes vitaux. <br /><br /> L'Ankylosaurus avait peu de dents, indiquant un faible pouvoir de mastication.",
    autres_infos: {
      taille: {
        longueur: '6-9 mètres',
        hauteur: '2 mètres',
      },
      poids: '4 tonnes',
    },
    decouverte: {
      date: '1906',
      lieu: 'États-Unis',
    },
  },
  {
    nom: 'Gomphotherium',
    etymologie: 'Animal à ongles',
    image_url: './assets/images/Gomphotherium.jpg',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: 'Chordata',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Mammalia',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Proboscidea',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Gomphotheriidae',
      sous_famille: '',
      tribu: '',
      sous_tribu: '',
      genre: 'Gomphotherium',
      espèces: 'Gomphotherium angustidens',
    },
    regime_alimentaire: 'Végétarien',
    habitat: 'Marécages',
    geologie: {
      ere: 'Cénozoïque',
      periode: 'Paléogène',
      epoque: 'Éocène',
      stage: 'Priabonien',
      apparition: -36,
      extinction: -23,
    },
    description:
      "Gomphotherium était un mammifère de grande taille, probablement similaire aux éléphants actuels. <br /> Il possédait quatre défenses presque droites et des molaires raboteuses. Les défences du bas se trouvaient le plus souvent tout près l'une de l'autre dans la mâchoire inférieure allongée et étroite, et elles ont pu être utilisées comme une pelle. Celles du haut étaient recouvertes d'une bande d'émail qui couvrait probablement toutes les défenses des proboscidiens anciens, mais qe l'on ne retrouve pas sur nos éléphants actuels. Elles font penser à des pics de mineurs et servaient probablement à creuser.",
    autres_infos: {
      taille: {
        longueur: '4 mètres',
        hauteur: '3 mètres',
      },
      poids: '2-4 tonnes',
    },
    decouverte: {
      date: '1971',
      lieu: 'Allemagne',
    },
  },
  {
    nom: 'Dodo',
    etymologie: "Du portugais 'doudo' qui signifie 'stupide' ou 'maladroit'",
    image_url: './assets/images/Dodo.jpeg',
    date_ajout: '2024-05-05',
    taxonomie: {
      règne: 'Animalia',
      sous_règne: '',
      rameau: '',
      infra_règne: '',
      super_embranchement: '',
      embranchement: '',
      sous_embranchement: 'Vertebrata',
      infra_embranchement: '',
      micro_embranchement: '',
      super_classe: '',
      classe: 'Aves',
      sous_classe: '',
      division: '',
      super_ordre: '',
      ordre: 'Columbiformes',
      sous_ordre: '',
      infra_ordre: '',
      micro_ordre: '',
      super_famille: '',
      famille: 'Columbidae',
      sous_famille: 'Raphinae',
      tribu: '',
      sous_tribu: '',
      genre: 'Raphus',
      espèces: 'Raphus cucullatus',
    },
    regime_alimentaire: 'Omnivore',
    habitat: 'Forêts',
    geologie: {
      ere: 'Cénozoïque',
      periode: '',
      epoque: '',
      stage: '',
      apparition: '',
      extinction: 1662,
    },
    description:
      "Le dodo était un oiseau trapu, incapable de voler, avec un bec crochu, des pattes courtes et une queue en forme de boule. Son plumage était gris-brun sur le dos et blanc sur le ventre. Il mesurait environ 1 mètre de haut et pesait jusqu'à 20 kg. Il a disparu du fait de la colonisation de l'Île Maurice. Très pataud au déplacement, il était facile de le capturer.",
    autres_infos: {
      taille: {
        longueur: '1 mètre',
        hauteur: '1 mètre',
      },
      poids: '20 kg',
    },
    decouverte: {
      date: '1598',
      lieu: 'Île Maurice',
    },
  },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
  // {
  //   nom: "",
  //   etymologie: "",
  //   image_url: "./assets/images/.jpg",
  //   taxonomie: {
  //     règne: "Animalia",
  //     sous_règne: "",
  //     rameau: "",
  //     infra_règne: "",
  //     super_embranchement: "",
  //     embranchement: "Chordata",
  //     sous_embranchement: "Vertebrata",
  //     infra_embranchement: "",
  //     micro_embranchement: "",
  //     super_classe: "",
  //     classe: "",
  //     sous_classe: "",
  //     division: "",
  //     super_ordre: "",
  //     ordre: "",
  //     sous_ordre: "",
  //     infra_ordre: "",
  //     micro_ordre: "",
  //     super_famille: "",
  //     famille: "",
  //     sous_famille: "",
  //     tribu: "",
  //     sous_tribu: "",
  //     genre: "",
  //     espèces: "",
  //   },
  //   regime_alimentaire: "",
  //   habitat: "",
  //   geologie: {
  //     ere: "ozoïque",
  //     periode: "",
  //     epoque: "",
  //     stage: "",
  //     apparition: ,
  //     extinction: ,
  //   },
  //   description: "",
  //   autres_infos: {
  //     taille: {
  //       longueur: "",
  //       hauteur: "",
  //     },
  //     poids: "",
  //   },
  //   decouverte: {
  //     date: "",
  //     lieu: "",
  //   },
  // },
];

export default data;
